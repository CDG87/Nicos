...
--
-- Client :  127.0.0.1
-- Généré le :  Lun 22 Janvier 2018 à 15:09
...
--
DELIMITER $$
CREATE TRIGGER `incre_audit` BEFORE INSERT ON `audit` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `incre_batiment` BEFORE INSERT ON `batiment` FOR EACH ROW BEGIN
	DECLARE increbatiment INTEGER;
	SET increbatiment = (SELECT COALESCE(MAX(NUM_BATIMENT), 0) + 1 FROM batiment);
	SET NEW.NUM_BATIMENT = increbatiment;
END
$$
DELIMITER ;
...
--
-- Déclencheurs `controleur`
--
DELIMITER $$
CREATE TRIGGER `incre_controleur` BEFORE INSERT ON `controleur` FOR EACH ROW BEGIN
	DECLARE increcontroleur INTEGER;
	SET increcontroleur = (SELECT COALESCE(MAX(NUM_CONTROLEUR), 0) + 1 FROM controleur);
	SET NEW.NUM_CONTROLEUR = increcontroleur;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_critere` BEFORE INSERT ON `critere` FOR EACH ROW BEGIN
	DECLARE increcritere INTEGER;
    DECLARE increresart INTEGER;
	SET increcritere = (SELECT COALESCE(MAX(NUM_CRITERE), 0) + 1 FROM critere);
    SET increresart = (SELECT COALESCE(MAX(NUM_RESUME_ARTICLE), 0) + 1 FROM critere);
	SET NEW.NUM_CRITERE = increcritere;
    SET NEW.NUM_RESUME_ARTICLE = increresart;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_groupe_lieu` BEFORE INSERT ON `groupe_lieu` FOR EACH ROW BEGIN
	DECLARE incregroupelieu INTEGER;
	SET incregroupelieu = (SELECT COALESCE(MAX(NUM_GROUPE_LIEU), 0) + 1 FROM groupe_lieu);
	SET NEW.NUM_GROUPE_LIEU = incregroupelieu;
END
$$
DELIMITER ;
...
--
-- Déclencheurs `image_critere`
--
DELIMITER $$
CREATE TRIGGER `incre_image_critere` BEFORE INSERT ON `image_critere` FOR EACH ROW BEGIN
	DECLARE increimagecritere INTEGER;
	SET increimagecritere = (SELECT COALESCE(MAX(NUM_IMAGE_CRITERE), 0) + 1 FROM image_critere);
	SET NEW.NUM_IMAGE_CRITERE = increimagecritere;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_lieu` BEFORE INSERT ON `lieu` FOR EACH ROW BEGIN
	DECLARE increlieu INTEGER;
	SET increlieu = (SELECT COALESCE(MAX(NUM_LIEU), 0) + 1 FROM lieu);
	SET NEW.NUM_LIEU = increlieu;
END
$$
DELIMITER ;
...
INSERT INTO `observation` (`NUM_OBSERVATION`, `NUM_CRITERE`, `LIBELLE_OBSERVATION`, `CODE_COULEUR_OBSERVATION`, `code_obs`, `DATE_OBSERVATION`) VALUES
(4, 1, 'L’Autorité Territoriale n’a pas désigné d’agent sur la mission d\'Assistant de Prévention.', 2, NULL, NULL),
(6, 2, 'Le ou les agents désignés en tant qu\'Assistant de Prévention disposent d\'un arrêté de désignation.', 1, NULL, NULL),
(7, 2, 'Le ou les agents désignés en tant qu\'Assistant de Prévention ne disposent pas d\'un arrêté de désignation.a', 2, NULL, NULL),
(8, 3, 'Le ou les agents disposent de leur lettre de cadrage.', 1, NULL, NULL),
(10, 3, 'Le ou les agents désignés ne disposent pas de leur lettre de cadrage .', 2, NULL, NULL),
(12, 4, 'FORMATION OBLIGATOIRE:\r\nLe ou les agents ont suivi la formation obligatoire d\'Assistant de Prévention.', 1, NULL, NULL),
(15, 4, 'FORMATION OBLIGATOIRE:\r\nIl a été indiqué que le ou les agents n\'ont pas suivi la formation obligatoire.', 2, NULL, NULL),
(16, 5, 'Les moyens mis à disposition du ou des agents pour assurer les missions d\'AP sont suffisants pour assurer toutes les activités définies dans la lettre de cadrage.', 1, NULL, NULL),
(18, 5, 'Les moyens mis à disposition du ou des agents pour assurer les missions d\'AP ne sont pas suffisants pour assurer toutes les activités définies dans la lettre de cadrage.', 2, NULL, NULL),
(21, 6, 'L\'Autorité Territoriale a désigné un agent sur la mission de Conseiller de Prévention.', 1, NULL, NULL),
(23, 6, 'L\'Autorité Territoriale n\'a pas désigné d\'agent sur la mission de Conseiller de Prévention.', 2, NULL, NULL),
(25, 7, 'L\'agent désigné en tant que Conseiller de Prévention dispose d\'un arrêté de nomination.', 1, NULL, NULL),
(26, 7, 'L\'agent désigné en tant que Conseiller de Prévention ne dispose pas d\'un arrêté de nomination.', 2, NULL, NULL),
(28, 8, 'L\'agent dispose d\'une lettre de cadrage.', 1, NULL, NULL),
(30, 8, 'L\'agent ne dispose pas d\'une lettre de cadrage.', 2, NULL, NULL),
(32, 9, 'FORMATION OBLIGATOIRE:\r\nL\'agent a suivi la formation initiale et est à jour de sa formation continue.', 1, NULL, NULL),
(34, 9, 'FORMATION OBLIGATOIRE:\r\nIl a été indiqué que l\'agent n\'a pas suivi la formation obligatoire préalable à la prise de fonction.', 2, NULL, NULL),
(36, 10, 'Les moyens mis à disposition de l\'agent pour assurer les missions de Conseiller de Prévention sont suffisants au regard des activités définies dans la lettre de cadrage.', 1, NULL, NULL),
(37, 10, 'Les moyens mis à disposition de l\'agent pour assurer les missions de Conseiller de Prévention ne sont pas suffisants au regard des activités définies dans la lettre de cadrage.', 2, NULL, NULL),
(40, 11, 'Il a été indiqué que les agents sont à jour de leur suivi médical.', 1, NULL, NULL),
(41, 11, 'Il a été indiqué que les agents ne sont pas tous à jour de leur suivi médical.', 2, NULL, NULL),
(42, 12, 'Le médecin de prévention a été consulté sur les projets de construction ou d\'aménagement importants des locaux de travail.', 1, NULL, NULL),
(43, 12, 'Il n\'y a pas eu de consultation du Médecin de prévention pour les projets de construction ou d\'aménagement importants des locaux de travail.', 2, NULL, NULL),
(44, 13, 'Le Médecin de Prévention est informé lors de l\'achat et de l\'utilisation de produits dangereux.', 1, NULL, NULL),
(45, 13, 'Le Médecin de Prévention n\'est pas informé lors de l\'achat et de l\'utilisation de produits dangereux.', 2, NULL, NULL),
(46, 14, 'Le service de médecine préventive est informé des accidents de service et/ou des maladies professionnelles.', 1, NULL, NULL),
(47, 14, 'Le service de médecine préventive n\'est pas informé des accidents de service et/ou des maladies professionnelles.', 2, NULL, NULL),
(48, 15, 'La liste des agents soumis à une surveillance médicale particulière a été communiquée au Médecin de Prévention.', 1, NULL, NULL),
...
(51, 16, 'Le rapport d\'activité a été établi et transmis à l\'Autorité Territoriale.', 1, NULL, NULL),
(53, 16, 'Le rapport d\'activité n\'a pas été transmis à l\'Autorité Territoriale.', 2, NULL, NULL),
(54, 17, 'Compte tenu de l\'effectif de la collectivité (<50 agents), les missions du CHSCT sont placées auprès du Comité Technique du CDG87.', 1, NULL, NULL),
(55, 17, 'Un Comité d\'Hygiène, de Sécurité et des Conditions de Travail (CHSCT) a été créé.', 1, NULL, NULL),
(56, 17, 'Le Comité d\'Hygiène, de Sécurité et des Conditions de Travail (CHSCT) n\'a pas été créé.', 2, NULL, NULL),
(57, 18, 'Les membres du CHSCT sont formés.', 1, NULL, NULL),
(58, 18, 'Les membres du CHSCT n\'ont pas été formés.', 2, NULL, NULL),
(59, 19, 'Le programme annuel de prévention a été soumis aux membres du comité.', 1, NULL, NULL),
(60, 19, 'Le programme annuel de prévention n\'a pas été soumis aux membres du comité.', 2, NULL, NULL),
(62, 20, 'Le CHSCT est informé de tout accident de service grave ou répétés et des suites données à chaque enquète.', 1, NULL, NULL),
(63, 20, 'Le CHSCT n\'est pas informé des accidents de service graves ou répétés.', 2, NULL, NULL),
(64, 21, 'Le CT est consulté pour avis sur les sujets d\'ordre général intéressant l\'hygiène, la sécurité et les conditions de travail.', 1, NULL, NULL),
(65, 21, 'Le CHSCT est consulté pour avis sur les sujets d\'ordre général intéressant l\'hygiène, la sécurité et les conditions de travail.', 1, NULL, NULL),
(66, 21, 'Le CT n\'est pas consulté pour avis sur les sujets d\'ordre général intéressant l\'hygiène, la sécurité et les conditions de travail.', 2, NULL, NULL),
(67, 21, 'Le CHSCT n\'est pas consulté pour avis sur les sujets d\'ordre général intéressant l\'hygiène, la sécurité et les conditions de travail.', 2, NULL, NULL),
(68, 22, 'Le comité à été réuni conformémént à la règlementation.', 1, NULL, NULL),
(69, 22, 'Le Comité n\'a pas été réuni conformément à la règlementation.', 2, NULL, NULL),
(70, 23, 'Le règlement intérieur du Comité à été créé.', 1, NULL, NULL),
(71, 23, 'Le règlement intérieur du Comité n\'a pas été rédigé.', 2, NULL, NULL),
(72, 24, 'Les comptes-rendus ont été établis.', 1, NULL, NULL),
(73, 24, 'Les compte-rendus n\'ont pas été établis.', 2, NULL, NULL),
(74, 25, 'Le comité à été informé de la visite et des observations  de l\'ACFI.', 1, NULL, NULL),
(75, 25, 'Le comité à été informé de la visite de l\'ACFI mais pas de ses observations.', 2, NULL, NULL),
(76, 25, 'Le comité n\'a pas été informé de la visite et des observations de l\'ACFI.', 2, NULL, NULL),
(77, 26, 'Un ACFI à été désigné.', 1, NULL, NULL),
(78, 26, 'Il n\'y a pas d\'ACFI désigné.', 2, NULL, NULL),
(79, 27, 'Une convention et une lettre de mission ont été rédigées.', 1, NULL, NULL),
(80, 27, 'La convention et la lettre de mission n\'ont pas été rédigées.', 2, NULL, NULL),
(81, 28, 'L\'ACFI est consulté sur la teneur de tous documents se rattachant à sa mission, et notamment des règlements et des consignes que l\'autorité territoriale envisage d\'adopter en matière d\'hygiène, de sécurité et de conditions de travail.', 1, NULL, NULL),
(82, 28, 'L\'ACFI n\'est pas consulté sur la teneur de tous documents se rattachant à sa mission, et notamment des règlements et des consignes que l\'autorité territoriale envisage d\'adopter en matière d\'hygiène, de sécurité et de conditions de travail.', 2, NULL, NULL),
(83, 29, 'L\'ACFI a effectué un contrôle des conditions d\'application des règles d\'hygiène et de sécurité.', 1, NULL, NULL),
(84, 29, 'L\'ACFI n\'a pas effectué un contrôle des conditions d\'application des règles d\'hygiène et de sécurité.', 2, NULL, NULL),
(85, 30, 'L\'autorité territoriale a informé l\'ACFI des suites données à ses propositions.', 1, NULL, NULL),
(86, 30, 'L\'autorité territoriale n\'a pas informé l\'ACFI des suites données à ses propositions.', 2, NULL, NULL),
(88, 31, 'Le document unique d\'évaluation des risques à été réalisé.', 1, NULL, NULL),
(89, 31, 'Le document unique d\'évaluation des risques n\'a pas été réalisé.', 2, NULL, NULL),
(90, 31, 'Il a été indiqué que le document unique d\'évaluation des risques (DU-EvRP) est en cours de réalisation.', 2, NULL, NULL),
(92, 32, 'Le document est mis à jour chaque fois que nécessaire et au moins tous les ans.', 1, NULL, NULL),
(93, 32, 'Il a été indiqué que le document unique n\'a pas été mis à jour depuis plus d\'un an.', 2, NULL, NULL),
(95, 33, 'Le document unique est mis à disposition et son emplacement est indiqué.', 1, NULL, NULL),
(96, 33, 'Le document unique est mis à disposition des agents mais son emplacement n\'est pas indiqué.', 2, NULL, NULL),
(97, 33, 'Il a été indiqué que le document unique n\'est pas mis à disposition des agents et des acteurs de la prévention.', 2, NULL, NULL),
(99, 34, 'Le document unique n\'a pas été utilisé pour l\'élaboration du programme annuel de prévention.', 2, NULL, NULL),
(100, 34, 'Le document unique a été utilisé pour l\'élaboration du programme annuel de prévention.', 1, NULL, NULL),
(101, 35, 'Les RPS ont été évalués dans le document unique.', 1, NULL, NULL),
(102, 35, 'Les RPS n\'ont pas été évalués.', 2, NULL, NULL),
(103, 35, 'Les RPS n\'ont été évalués que succintement dans le document unique.', 2, NULL, NULL),
(104, 36, 'Il a été indiqué qu\'un registre Santé et Sécurité au Travail a été ouvert par service.', 1, NULL, NULL),
(105, 36, 'Il a été indiqué qu\'aucun registre Santé et Sécurité au Travail n\'a été ouvert.', 2, NULL, NULL),
(106, 37, 'Le registre spécial coté des dangers graves et imminents a été ouvert et est tenu à disposition des agents et membres du comité.', 1, NULL, NULL),
(107, 37, 'Il a été indiqué qu\'aucun registre Santé et Sécurité au Travail n\'a été ouvert.', 2, NULL, NULL),
(108, 38, 'Le registre de sécurité a été ouvert.', 1, NULL, NULL),
(109, 38, 'Il a été indiqué que le registre de sécurité n\'a pas été ouvert.', 2, NULL, NULL),
(110, 39, 'Il a été indiqué qu\'un registre sécurité incendie a été ouvert pour chaque bâtiment classé ERP.', 1, NULL, NULL),
(111, 39, 'Il a été indiqué qu\'aucun registre sécurité incendie n\'a été ouvert pour chaque bâtiment classé ERP.', 2, NULL, NULL),
(112, 40, 'La collectivité n\'a pas sollicité le médecin de prévention afin d\'établir la fiche des risques professionnels propres aux différentes activités de ses services.', 2, NULL, NULL),
(113, 40, 'La collectivité a établi une ou plusieurs fiches des risques professionnels propres aux différentes activités de ses services.', 1, NULL, NULL),
(114, 41, 'Les fiches d\'exposition à la pénibilté et les fiches d\'exposition à l\'amiante n\'ont pas été établies.', 2, NULL, NULL),
(115, 41, 'Les fiches d\'exposition à la pénibilté et les fiches d\'exposition à l\'amiante ont été établies.', 1, NULL, NULL),
(116, 42, 'Les agents disposent d\'autorisations de conduite pour la conduite des différents engins de chantier délivrées par l\'Autorité territoriale. ', 1, NULL, NULL),
(117, 42, 'Les agents ne disposent pas d\'autorisation de conduite délivrée par l\'Autorité territoriale.', 2, NULL, NULL),
(118, 42, 'Les autorisations de conduite ne sont pas signées par l\'Autorité Territoriale en cours de mandat.', 2, NULL, NULL),
(119, 43, 'Les diagnostics Amiante (DTA) ont été faits.', 1, NULL, NULL),
(120, 43, 'Les diagnostics Amiante n\'ont pas été faits sur tous les bâtiments (permis avant 1997) de la collectivité.', 2, NULL, NULL),
(121, 43, 'Il a été indiqué que la collectivité procède à la réalisation d\'un diagnostic (DTA) particulier pour tous nouveaux travaux.', 1, NULL, NULL),
(122, 43, 'Il a été indiqué que la collectivité ne procéde pas systématiquement à la réalisation d\'un diagnostic (DTA) particulier pour tous nouveaux travaux.', 2, NULL, NULL),
(123, 43, 'Le ou les DTA ont été réalisés avant  le 21 décembre 2012 et n\'ont pas été mis à jour conformément à la nouvelle méthodologie de repérage.', 2, NULL, NULL),
(124, 44, 'Les agents  intervenants sur les installations électriques sont formés et habilités.', 1, NULL, NULL),
(126, 44, 'Il a été indiqué qu\'il n\'y a pas d\'agent formé et habilité à intervenir sur les installations électriques.', 2, NULL, NULL),
(127, 45, 'RECENSEMENT:\r\nIl a été indiqué qu\'un travail de recensement des produits chimiques à été réalisé.', 1, NULL, NULL),
(128, 45, 'FDS:\r\nIl a été indiqué que les fiches de données sécurité des produits chimiques utilisés par les agents sont à disposition des agents .', 1, NULL, NULL),
(130, 45, 'RECENSEMENT:\r\nIl a été indiqué qu\'aucun recensement des produits chimiques n\'a été fait.', 2, NULL, NULL),
(131, 45, 'FDS:\r\nIl a été indiqué que les fiches de données de sécurité ne sont pas à disposition des agents sur les lieux de stockage.', 2, NULL, NULL),
(132, 46, 'Il a été indiqué que des notices techniques ont été mises à disposition des agents utilisateurs.', 1, NULL, NULL),
(133, 46, 'Il a été indiqué que les notices techniques des produits chimiques manipulés ne sont pas disponibles.', 2, NULL, NULL),
(134, 47, 'Les déclarations d\'intention de commencement de travaux (DICT) sont systématiquement faites.', 1, NULL, NULL),
(135, 47, 'Les déclarations d\'intention de commencement de travaux (DICT) ne sont pas faites.', 2, NULL, NULL),
(136, 48, 'Il a été indiqué que ces équipements et installations dont la réglementation l\'impose sont vérifiés périodiquement. ', 1, NULL, NULL),
(137, 48, 'Il a été indiqué qu\'il n\'y a pas de vérifications périodiques de ces installations.', 2, NULL, NULL),
(138, 49, 'Le contrôle périodique de ces installations est effectué par un organisme agréé.', 1, NULL, NULL),
(139, 49, 'Il a été indiqué que le contrôle périodiquement n\'a pas été effectué par un organisme agréé.', 2, NULL, NULL),
(140, 50, 'Les appareils et/ou accessoires de levage sont contrôlés périodiquement.', 1, NULL, NULL),
(141, 50, 'Il a été indiqué que les appareils ou accessoires de levage ne sont pas contrôlés périodiquement.', 2, NULL, NULL),
(142, 51, 'Les installations thermiques sont contrôlées et entretenues périodiquement par un prestataire extérieur dans le cadre d\'un contrat d\'entretien.', 1, NULL, NULL),
(143, 51, 'Il a été indiqué que les installations thermiques n\'ont jamais été contrôlées dans le cadre d\'un contrat d\'entretien.', 2, NULL, NULL),
(145, 52, 'Le ou les échafaudages, échelles et escabeaux font l\'objet d\'une vérification périodique.', 1, NULL, NULL),
(146, 52, 'Il a été indiqué que le ou les échafaudages, échelles et escabeaux ne font pas l\'objet d\'une vérification périodique.', 2, NULL, NULL),
(148, 53, 'Les installations électriques sont vérifiées périodiquement.', 1, NULL, NULL),
(149, 53, 'Il a été indiqué que les installations électriques ne sont pas vérifiées.', 2, NULL, NULL),
(150, 54, 'Le ou les compresseurs ne sont pas contrôlés.', 2, NULL, NULL),
(151, 54, 'Le ou les compresseurs sont contrôlés.', 1, NULL, NULL),
(152, 54, 'Le ou les compresseurs de plus de 10 ans n\'ont pas fait l\'objet d\'une requalification.', 2, NULL, NULL),
(153, 55, 'La vérification périodique des EPI soumis à un contrôle n\'est pas faite.', 2, NULL, NULL),
(154, 55, 'La vérification périodique des EPI soumis à  contrôle est faite.', 1, NULL, NULL),
(155, 55, 'Tous les EPI soumis à un contrôle n\'ont pas fait l\'objet d\'une vérification périodique. (Voir la liste dans les préconisations)', 2, NULL, NULL),
(158, 56, 'Il a été indiqué que la collectivité ne dispose pas d\'équipement concerné par cette disposition. ', 2, NULL, NULL),
(159, 56, 'Il a été indiqué que la collectivité dispose d\'équipement concerné par cette disposition. ', 1, NULL, NULL),
(160, 57, 'EXTINCTEURS:\r\nLes contrôles périodiques des extincteurs sont faits.', 1, NULL, NULL),
(161, 57, 'EXTINCTEURS:\r\nLe contrôles périodique des extincteurs n\'est pas fait.', 2, NULL, NULL),
(162, 57, 'ALARME INCENDIE:\r\nLes vérifications des systèmes d\'alarme incendie on été faites.', 1, NULL, NULL),
(163, 57, 'ALARME INCENDIE:\r\nLes vérifications des systèmes d\'alarme incendie ne sont pas faites.', 2, NULL, NULL),
(166, 58, 'PTAC<3,5T:\r\nLes contrôles techniques de tous les véhicules (légers et utilitaires) sont effectués annuellement.', 1, NULL, NULL),
(169, 59, 'Ces installations sont contrôlées périodiquement.', 1, NULL, NULL),
(170, 59, 'Il a été indiqué que ces installations ne sont pas contrôlées périodiquement.', 2, NULL, NULL),
(171, 60, 'Il a été indiqué que la vérification périodique du ou des réservoirs n\'ont pas été faites.', 2, NULL, NULL),
(172, 60, 'La vérification périodique du ou des réservoirs a été faite.', 1, NULL, NULL),
(173, 61, 'Les vérifications périodiques ont été effectuées.', 1, NULL, NULL),
(174, 61, 'Il a été indiqué que les vérifications périodiques n\'ont pas été effectuées.', 2, NULL, NULL),
(175, 62, 'Il a été indiqué que les formations initiales à la sécurité étaient effectuées pour chaque nouvel entrant.', 1, NULL, NULL),
(176, 62, 'Il a été indiqué qu\'aucune formation initiale à la sécurité n\'était effectuée.', 2, NULL, NULL),
(177, 63, 'Un plan de formation en tant que tel n\'existe pas formellement mais un suivi tracé des formations à recyclage existe.', 1, NULL, NULL),
(179, 64, 'Tous les agents ont suivi une formation à l\'utilisation des extincteurs.', 1, NULL, NULL),
(180, 64, 'Il a été indiqué qu\'aucun agent n\'a suivi de formation à l\'utilisation des extincteurs.', 2, NULL, NULL),
(182, 65, 'Une organisation a été mise en place pour qu\'au moins deux agents de chaque services où sont effectués des travaux dangereux suivent cette formation.', 1, NULL, NULL),
(183, 65, 'Il a été indiqué qu\'il n\'y a pas d\'agent formé aux premiers secours dans les services où sont effectués des travaux dangereux .', 2, NULL, NULL),
(184, 65, 'Il a été indiqué qu\'il n\'y a pas d\'agent formé dans tous les services où sont effectués des travaux dangereux.', 2, NULL, NULL),
(185, 66, 'Un suivi des formations des agents au poste de travail est en place dans la structure.', 1, NULL, NULL),
(186, 66, 'La collectivité a mis en place certaines actions de communication/information vers les agents,telles que l\'utilisation des EPIs et/ou la procédure d\'utilisation des produits d\'entretien', 1, NULL, NULL),
(189, 66, 'Il a été indiqué que les agents n\'ont pas suivi de formation sur les risques spécifiques aux différentes activités.', 2, NULL, NULL),
(191, 67, 'TRACTEUR>50CV:\r\nIl a été indiqué que pour la conduite en sécurité du ou des tracteurs >50cv, le ou les agents n\'ont pas suivi de formation.', 2, NULL, NULL),
(192, 67, 'TRACTEUR>50CV:\r\nPour la conduite en sécurité du ou des tracteurs >50cv, le ou les agents ont suivi la formation CACES 8 et sont à jour de leur recyclage.', 1, NULL, NULL),
(193, 67, 'MINIPELLE/TRACTEUR<50CV:\r\nPour la conduite en sécurité du ou des minipelles/tracteurs <50cv, le ou les agents ont suivi la formation CACES 1 et sont à jour de leur recyclage.', 1, NULL, NULL),
(194, 67, 'TRACTOPELLE:\r\nPour la conduite en sécurité du ou des tractopelles, le ou les agents ont suivi la formation CACES 4 et sont à jour de leur recyclage.', 1, NULL, NULL),
(195, 67, 'PEMP:\r\nPour la conduite en sécurité de la nacelle élévatrice, le ou les agents ont suivi la formation CACES PEMP et sont à jour de leur recyclage.', 1, NULL, NULL),
(197, 67, 'TONDEUSE AUTOPORTEE:\r\nPour la conduite en sécurité des tondeuses autoportées, le ou les agents ont suivi une formation spécifique.', 1, NULL, NULL),
(198, 68, 'Le ou les agents concernées par les travaux à proximité des réseaux aériens ou enterrés ont obtenu l\'habilitation AIPR (autorisation d’intervention à proximité des réseaux)', 1, NULL, NULL),
(199, 68, 'Il a été indiqué que le ou les agents concernées par les travaux à proximité des réseaux aériens ou enterrés ne possèdent pas l\'habilitation AIPR (autorisation d’intervention à proximité des réseaux)', 2, NULL, NULL),
(200, 69, 'CERTIPHYTO:\r\nLe ou les agents qui utilisent des produits phyto-sanitaires ont suivi la formation Certi-Phyto. (applicateur et applicateur opérationnel).', 1, NULL, NULL),
(201, 69, 'CERTIBIOCIDE:\r\nLe ou les agents qui utilisent les produits biocides ont suivi la formation Certi-biocides.', 1, NULL, NULL),
(202, 69, 'CERTIPHYTO:\r\nIl a été indiqué que le ou les agents qui utilisent les produits phyto-sanitaires n\'ont pas suivi la formation Certi-Phyto.', 2, NULL, NULL),
(203, 69, 'CERTIBIOCIDE:\r\nIl a été indiqué que le ou les agents qui utilisent les produits biocides n\'ont pas suivi la formation Certi-biocides.', 2, NULL, NULL),
(204, 70, 'Un ou des agents sont formés aux techniques de tronçonnage.', 1, NULL, NULL),
(205, 70, 'Il a été indiqué qu\'il n\'y a pas d\'agent formé aux techniques de tronçonnage.', 2, NULL, NULL),
(206, 71, 'Il a été indiqué que les agents susceptibles d\'être exposés à l\'amiante ont suivi la formation réglementaire.', 1, NULL, NULL),
(207, 71, 'Il a été indiqué que les agents susceptibles d\'être exposés à l\'amiante n\'ont pas suivi la formation réglementaire.', 2, NULL, NULL),
(208, 72, 'LOCAUX:\r\nIl a été indiqué que tous les locaux où interviennent les agents sont pourvus d\'une trousse de premiers secours .', 1, NULL, NULL),
(209, 72, 'LOCAUX:\r\nIl a été indiqué que les locaux où interviennent les agents ne sont pas pourvus de trousse de premiers secours .', 2, NULL, NULL),
(210, 72, 'VÉHICULES:\r\nIl a été indiqué que les véhicules de service sont équipés de trousse de secours.', 1, NULL, NULL),
(211, 72, 'VÉHICULES:\r\nIl a été indiqué que les véhicules de service ne sont pas équipés de trousse de secours. ', 2, NULL, NULL),
(213, 73, 'Il a été indiqué que l\'affichage des différents n° d\'urgence et consignes de sécurité n\'est pas présent sur tous les sites.', 2, NULL, NULL),
(214, 73, 'Il a été indiqué que l\'affichage des différents n° d\'urgence et consignes de sécurité est présent sur tous les sites.', 1, NULL, NULL),
(215, 73, 'Il a été indiqué qu\'un affichage d\'urgence est présent sur certains sites et qu\'il est nécessaire de le compléter. (voir les préconisations)', 2, NULL, NULL),
(216, 74, 'LOCAUX:\r\nIl a été indiqué que tous les locaux sont équipés d\'extincteurs.', 1, NULL, NULL),
(217, 74, 'LOCAUX:\r\nIl a été indiqué que tous les locaux ne sont pas équipés d\'extincteurs.', 2, NULL, NULL),
(218, 74, 'VÉHICULES:\r\nIl a été indiqué que les véhicules qui présentent un risque d\'incendie sont équipés d\'un extincteur.', 1, NULL, NULL),
(219, 74, 'VÉHICULES:\r\nIl a été indiqué que les véhicules qui présentent un risque d\'incendie ne sont pas équipés d\'un extincteur.', 2, NULL, NULL),
(220, 75, 'Il a été indiqué qu\'une organisation de travail est mise en place pour prévenir des risques liés aux situations de travail isolé.\r\nCette procédure est également en place pour les interventions sous astreinte', 1, NULL, NULL),
(222, 75, 'Il a été indiqué qu\'aucune organisation de travail n\'est mise en place pour prévenir les situations de travail isolé sur des activités à risques élevés. (tronçonnage, travail en hauteur, travail en bordure de bassin etc...)', 2, NULL, NULL),
(223, 76, 'SUR SITE:\r\nIl a été indiqué que les sites où peuvent intervenir les agents en situation de travailleur isolé sont équipés d\'un téléphone fixe permettant d\'accéder au réseau urbain.', 1, NULL, NULL),
(224, 76, 'EN EXTERIEUR:\r\nIl a été indiqué que les agents en situation de travailleur isolé en extérieur, ont à disposition des téléphones portables professionnels.', 1, NULL, NULL),
(225, 76, 'SUR SITE:\r\nIl a été indiqué que certains sites où peuvent intervenir les agents en situation de travailleur isolé ne sont pas équipés de moyen de communication.', 2, NULL, NULL),
(226, 76, 'EN EXTÉRIEUR:\r\nIl a été indiqué que un ou des agents en situation de travailleur isolé en extérieur, n\'ont pas à leur disposition de moyen de communication professionnel.', 2, NULL, NULL),
(227, 77, 'Les locaux de travail ont été rendus accessibles aux travailleurs handicapés.', 1, NULL, NULL),
(228, 77, 'Les locaux de travail n\'ont pas été rendus accessibles aux travailleurs handicapés.', 2, NULL, NULL),
(229, 78, 'Il a été indiqué que les équipements de travail et de protection individuelle sont fournis aux agents.\r\n\r\n', 1, NULL, NULL),
(230, 78, 'Il a été indiqué qu\'aucun équipement de travail et de protection individuelle n\'est fourni aux agents.', 2, NULL, NULL),
(231, 79, 'Il a été indiqué que les EPI sont  portés par les agents.', 1, NULL, NULL),
(232, 79, 'Il a été indiqué que les EPI ne sont pas systématiquement portés par les agents.', 2, NULL, NULL),
(233, 80, 'Le suivi est assuré par chaque agent avant utilisation.\r\nUne mise à jour régulière des équipements est réalisée en fonction de leur état mais il n\'y a pas de contrôle systématique mis en place.', 1, NULL, NULL),
(234, 80, 'Le suivi et l\'entretien ne sont pas maîtrisés, il a été indiqué que les agents gèrent eux-même le remplacement et le nettoyage de leurs EPI.', 2, NULL, NULL),
(235, 81, 'FORMATION:\r\nUne formation a été faite au personnel sur la nécessité et l\'entrainement au port des EPI.', 1, NULL, NULL),
(236, 81, 'FORMATION:\r\nAbsence de formation au port des EPI.', 2, NULL, NULL),
(237, 82, 'Des consignes d\'utilisation des EPI ont été élaborées.', 1, NULL, NULL),
(238, 82, 'Absence de consigne d\'utilisation des EPI.', 2, NULL, NULL),
(239, 82, 'Les consignes d\'utilisation de certains équipements n\'ont pas été élaborées.', 2, NULL, NULL),
(240, 83, 'Il a été indiqué que le ou les jeunes embauchés de moins de 18 ans n\'effectuent pas de travaux interdits.', 1, NULL, NULL),
(241, 83, 'Le ou les jeunes embauchés de moins de 18 ans effectuent un ou des travaux interdits.', 2, NULL, NULL),
(242, 84, 'Le ou les jeunes travailleurs âgés de 15 à moins de 18 ans  sont apprentis ou titulaires d’un contrat de professionnalisation ou stagiaires de la formation professionnelle ou élèves ou étudiants préparant un diplôme professionnel ou technologique.', 1, NULL, NULL),
(243, 84, 'Le ou les jeunes travailleurs âgés de 15 à moins de 18 ans ne sont pas apprentis ou titulaires d’un contrat de professionnalisation ou stagiaires de la formation professionnelle ou élèves ou étudiants préparant un diplôme professionnel ou technologique.', 2, NULL, NULL),
(244, 85, 'Les conditions de dérogation ont été satisfaites.', 1, NULL, NULL),
(245, 85, 'Les conditions de dérogation n\'ont pas été satisfaites.', 2, NULL, NULL),
(246, 86, 'La délibération a été prise par l\'organe délibérant.', 1, NULL, NULL),
(247, 86, 'La délibération n\'a pas été prise par l\'organe délibérant.', 2, NULL, NULL),
(248, 87, 'La délibération a été transmise pour information au Comité D’hygiène de Sécurité et des Conditions de Travail (CHSCT) ni adressée à l’Agent Chargé de la Fonction d’Inspection (ACFI).', 1, NULL, NULL),
(249, 87, 'La délibération n\'a pas été transmise pour information au Comité D’hygiène de Sécurité et des Conditions de Travail (CHSCT) et adressée à l’Agent Chargé de la Fonction d’Inspection (ACFI).', 2, NULL, NULL),
(250, 87, 'La délibération n\'a pas été transmise pour information au Comité D’hygiène de Sécurité et des Conditions de Travail (CHSCT).', 2, NULL, NULL),
(251, 87, 'La délibération n\'a pas été adressée à l’Agent Chargé de la Fonction d’Inspection (ACFI).', 2, NULL, NULL),
(252, 88, 'La délibération a été renouvelée au bout de 3 ans.', 1, NULL, NULL),
(253, 88, 'La délibération n\'a pas été renouvelée au bout de 3 ans.', 2, NULL, NULL),
(254, 89, 'Il n\'existe pas de procédure mise en place pour l\'accueil d\'entreprises extérieures et la rédaction de plan de prévention.', 2, NULL, NULL),
(255, 89, 'Il existe une procédure mise en place pour l\'accueil d\'entreprises extérieures et la rédaction de plan de prévention.', 1, NULL, NULL),
(256, 90, 'Il n\'existe pas de procédure d\'élaboration d\'un protocole sécurité (chargement/déchargement) avec les fournisseurs/livreurs.', 2, NULL, NULL),
(257, 90, 'Il existe une procédure d\'élaboration d\'un protocole sécurité (chargement/déchargement) avec les fournisseurs/livreurs.', 1, NULL, NULL),
(261, 91, 'Une coordination sécurité et protection de la santé (CSPS) est prévue à l’occasion de tous chantiers importants impliquant de la coactivité.', 1, NULL, NULL),
(262, 91, 'Aucune coordination sécurité et protection de la santé (CSPS) n\'est prévue à l’occasion des chantiers importants impliquant de la coactivité.', 2, NULL, NULL),
(263, 92, 'L\'autorité territoriale ne délivre pas de Permis de feu lorsque les agents interviennent pour des travaux par "point chaud" dans les batiments autres que le service technique.', 2, NULL, NULL),
(264, 92, 'L\'autorité territoriale délivre un Permis de feu lorsque les agents interviennent pour des travaux par "point chaud" dans les batiments autres que le service technique.', 1, NULL, NULL),
(265, 93, 'Pas de signalisation des zones de danger', 2, NULL, NULL),
(266, 93, 'La signalisation des zones de danger est incomplète', 2, NULL, NULL),
(267, 93, 'La signalisation des zones de danger est conforme', 1, NULL, NULL),
(273, 94, 'VESTIAIRES/SANITAIRES:\r\nLe nettoyage des vestiaires et des installations sanitaires est à revoir.', 2, NULL, NULL),
(274, 94, 'NETTOYAGE:\r\nLes locaux de travail sont entretenus et nettoyés régulièrement.', 1, NULL, NULL),
(275, 94, 'LOCAUX DE TRAVAIL:\r\nL’entretien et le nettoyage sont à revoir.', 2, NULL, NULL),
(276, 95, 'Absence de point d’eau potable et fraîche.', 2, NULL, NULL),
(277, 95, 'Eau potable et fraîche à disposition.', 1, NULL, NULL),
(278, 96, 'INTERDICTION:\r\nL’interdiction de fumer n’est pas respectée.', 2, NULL, NULL),
(279, 96, 'AFFICHAGE:\r\nL’affichage réglementaire n’est pas présent.', 2, NULL, NULL),
(281, 96, 'INTERDICTION:\r\nL’interdiction de fumer est respectée.', 1, NULL, NULL),
(282, 97, 'VESTIAIRES:\r\nLes vestiaires sont installés dans un local spécial de surface convenable(au moins 1m² par agent)', 1, NULL, NULL),
(284, 97, 'LAVABOS:\r\nLe nombre de lavabos est conforme (1 pour 10 agents au plus)', 1, NULL, NULL),
(286, 98, '(Une ou) des douches sont à disposition (du ou) des agents du centre technique.', 1, NULL, NULL),
(287, 98, 'Aucune douche n’est à disposition des agents du centre technique.', 2, NULL, NULL),
(288, 99, 'LOCAL:\r\nAbsence de local de restauration.', 2, NULL, NULL),
(289, 99, 'LOCAL:\r\nPrésence d’un local de restauration.', 1, NULL, NULL),
(290, 99, 'EQUIPEMENTS:\r\nAbsence de moyen de conservation ou de réfrigération des aliments et des boissons et d’une installation permettant de réchauffer les plats.', 2, NULL, NULL),
(291, 99, 'EQUIPEMENTS:\r\nPrésence de moyen de conservation ou de réfrigération des aliments et des boissons et d’une installation permettant de réchauffer les plats.', 1, NULL, NULL),
(292, 99, 'MOBILIER:\r\nNombre de sièges et de tables suffisant.', 1, NULL, NULL),
(293, 99, 'MOBILIER:\r\nNombre de sièges et de tables insuffisant.', 2, NULL, NULL),
(294, 100, 'CABINET D\'AISANCE:\r\nAbsence de cabinet d’aisance.', 2, NULL, NULL),
(295, 100, 'NOMBRE:\r\nCabinets d’aisance en nombre insuffisant.', 2, NULL, NULL),
(296, 100, 'ENTRETIEN:\r\nLa périodicité de nettoyage est insuffisante.', 2, NULL, NULL),
(297, 100, 'SÉPARATION:\r\nPas de cabinet d’aisance séparés pour le personnel féminin et masculin', 2, NULL, NULL),
(298, 100, 'CABINET D\'AISANCE:\r\nPrésence de cabinet d’aisance en nombre suffisant.', 1, NULL, NULL),
(299, 100, 'NETTOYAGE:\r\nLes installations sanitaires sont entretenues et nettoyées régulièrement.', 1, NULL, NULL),
(300, 100, 'SÉPARATION:\r\nLes toilettes homme / femme sont séparés', 1, NULL, NULL),
(301, 101, 'Absence d’eau potable et fraîche à disposition.', 2, NULL, NULL),
(302, 101, 'Eau potable et fraîche à disposition.', 1, NULL, NULL),
(303, 102, 'Absence de local de repos.', 2, NULL, NULL),
(304, 102, 'Présence d’un local de repos.', 1, NULL, NULL),
(305, 103, 'CHAUFFAGE:\r\nLes locaux affectés au travail sont chauffés pendant la saison froide.', 1, NULL, NULL),
(306, 103, 'CHAUFFAGE:\r\nLes locaux affectés au travail ne sont pas chauffés pendant la saison froide.', 2, NULL, NULL),
(307, 103, 'VÊTEMENTS:\r\nLes agents dont les activités les amènent à intervenir en extérieur, sont dotés de vêtements adaptés contre le froid.', 1, NULL, NULL),
(308, 103, 'VÊTEMENTS:\r\nLes agents dont les activités les amènent à intervenir en extérieur, ne sont pas dotés de vêtement adapté contre le froid.', 2, NULL, NULL),
(309, 104, 'ECLAIRAGE:\r\nL’éclairage des locaux de travail est suffisant.', 1, NULL, NULL),
(311, 104, 'ECLAIRAGE:\r\nL’éclairage des locaux de travail est insuffisant.', 2, NULL, NULL),
(312, 104, 'PROTECTION LUMINEUSE:\r\nLes ouvertures sont équipées de protections fixes ou mobiles appropriées contre les rayons solaires génants.', 1, NULL, NULL),
(313, 104, 'PROTECTION LUMINEUSE:\r\nLes ouvertures ne sont pas équipées de protection fixe ou mobile appropriée contre les rayons solaires gênants.', 2, NULL, NULL),
(314, 105, 'Absence d’aération par ventilation mécanique ou par ventilation naturelle permanente.', 2, NULL, NULL),
(315, 105, 'Présence d’une aération par ventilation mécanique ou par ventilation naturelle permanente.', 1, NULL, NULL),
(316, 106, 'VÉHICULES:\r\nLes véhicules de service sont équipés de trousse de secours', 1, NULL, NULL),
(317, 106, 'LOCAUX:\r\nLes locaux où interviennent les agents ne sont pas pourvus d\'une trousse de premiers secours', 2, NULL, NULL),
(318, 107, 'L\'affichage des différents numéros d\'urgence et consignes de sécurité est présent sur tous les sites de la structure.', 1, NULL, NULL),
(319, 107, 'L\'affichage des différents numéros d\'urgence et consignes de sécurité n\'est pas présent sur tous les sites de la structure.', 2, NULL, NULL),
(320, 108, 'TRAVAIL ISOLE:\r\nIl a été indiqué qu\'une organisation de travail est mise en place pour prévenir des risques liés aux situations de travail isolé.', 1, NULL, NULL),
(321, 108, 'TRAVAIL ISOLE:\r\nIl a été indiqué qu\'aucune organisation de travail n\'est mise en place pour prévenir les situations de travail isolé sur des activités à risques élevés. (tronçonnage, travail en hauteur, travail en bordure de bassin etc...)', 2, NULL, NULL),
(322, 109, 'TÉLÉPHONE FIXE:\r\nIl a été indiqué que le site est équipé d\'un téléphone fixe permettant d\'accéder au réseau urbain. ', 1, NULL, NULL),
(323, 109, 'TÉLÉPHONE FIXE:\r\nIl a été indiqué que le site n\'est pas équipé d\'un téléphone fixe permettant d\'accéder au réseau urbain. ', 2, NULL, NULL),
(324, 110, 'Absence de procédure d’urgence.', 2, NULL, NULL),
(325, 110, 'Présence d’une procédure d’urgence.', 1, NULL, NULL),
(326, 111, 'L’effectif théorique à été calculé.', 1, NULL, NULL),
(327, 111, 'L’effectif théorique n’a pas été calculé.', 2, NULL, NULL),
(328, 112, 'Les remarques de la commission de sécurité ont été levées.', 1, NULL, NULL),
(329, 112, 'Les remarques de la commission de sécurité n’ont pas été levées.', 2, NULL, NULL),
(330, 112, 'Un avis défavorable à été émis par la commission de sécurité.', 2, NULL, NULL),
(331, 113, 'NOMBRE:\r\nLes extincteurs sont en nombre suffisant', 1, NULL, NULL),
(332, 113, 'NOMBRE:\r\nLes extincteurs ne sont pas en nombre suffisant', 2, NULL, NULL),
(333, 113, 'ACCESSIBILITÉ:\r\nCertains extincteurs sont disposés trop en hauteur (poignée à plus de 1m50)', 2, NULL, NULL),
(334, 113, 'ACCESSIBILITÉ:\r\nLes extincteurs sont accessibles', 1, NULL, NULL),
(335, 113, 'SIGNALISATION:\r\nLa signalisation des extincteurs n’est pas conforme', 2, NULL, NULL),
(336, 113, 'SIGNALISATION:\r\nLa signalisation des extincteurs est conforme', 1, NULL, NULL),
(337, 113, 'ENGINS:\r\nLe ou les engins qui, avec leurs équipements (broyeur, épareuse...) présentent un risque d’incendie en sont équipés.', 1, NULL, NULL),
(338, 113, 'ENGINS:\r\nLe ou les engins qui, avec leurs équipements (broyeur, épareuse...) présentent un risque d’incendie ne sont pas équipés.', 2, NULL, NULL),
(339, 114, 'Des essais périodiques et des exercices d’évacuations ont lieu au moins tous les 6 mois.', 1, NULL, NULL),
(340, 114, 'Aucun exercice d’évacuation n’a été organisé.', 2, NULL, NULL),
(341, 114, 'Un seul exercice d’évacuation par an.', 2, NULL, NULL),
(342, 115, 'Les produits inflammables sont stockés dans un local (ou une armoire réservé à cet usage), ventilé, fermé à clé, clairement identifié, pourvu d\'un dispositif de rétention spécifique et résistant au feu.', 1, NULL, NULL),
(343, 115, 'Les produits inflammables sont stockés sans précaution particulière.', 2, NULL, NULL),
(344, 116, 'Les dispositifs de désenfumage sont présents.', 1, NULL, NULL),
(345, 116, 'Les dispositifs de désenfumage sont absents.', 2, NULL, NULL),
(346, 117, 'Les consignes incendie sont présentes.', 1, NULL, NULL),
(347, 117, 'Les consignes incendie sont absentes.', 2, NULL, NULL),
(348, 117, 'Les consignes incendie sont incomplètes.', 2, NULL, NULL),
(350, 118, 'AFFICHAGE:\r\nAbsence de plan d\'évacuation.', 2, NULL, NULL),
(351, 118, 'Les plans d\'évacuation sont à jour et affichés.', 1, NULL, NULL),
(352, 119, 'Le ou les dégagements sont libres.', 1, NULL, NULL),
(353, 119, 'Le ou les dégagements sont encombrés.', 2, NULL, NULL),
(355, 120, 'OUVERTURE:\r\nLa ou les portes ne disposent pas de dispositif dé-verrouillable sans clef', 2, NULL, NULL),
(356, 120, 'OUVERTURE:\r\nLa ou les portes disposent de dispositifs dé-verrouillables sans clef.', 1, NULL, NULL),
(357, 120, 'NOMBRE;\r\nLe nombre de dégagement est suffisant.', 1, NULL, NULL),
(358, 121, 'Le ou les escaliers sont munis de rampe ou de main-courante.', 1, NULL, NULL),
(359, 121, 'Absence de main-courante dans l\'escalier.', 2, NULL, NULL),
(360, 122, 'Le bâtiment est équipé d\'un système d\'alarme sonore.', 1, NULL, NULL),
(361, 122, 'Le système d\'alarme sonore est en dysfonctionnement.', 2, NULL, NULL),
(362, 123, 'Le ou les Blocs Autonomes d\'Eclairage Sécurité (BAES) sont en dysfonctionnement.', 2, NULL, NULL),
(363, 123, 'Le ou les Blocs Autonomes d\'Eclairage de Sécurité (BAES) sont en état de fonctionnement.', 1, NULL, NULL),
(364, 124, 'L\'évaluation du risque de chute de plain pied a été effectué.', 1, NULL, NULL),
(365, 124, 'Il n\'y a pas eu d\'évaluation du risque de chute de plain pied.', 2, NULL, NULL),
(366, 125, 'La fosse de visite est recouverte en dehors des interventions.', 1, NULL, NULL),
(367, 125, 'La fosse de visite n\'est pas conforme aux recommandations R468 ET R469.', 2, NULL, NULL),
(368, 126, 'La zone présente des risques de chute de plain pied', 2, NULL, NULL),
(369, 126, 'La zone ne présente pas de risque de chute', 1, NULL, NULL),
(370, 127, 'La zone est encombrée', 2, NULL, NULL),
(371, 127, 'La zone est exempt de tout encombrement', 1, NULL, NULL),
(372, 128, 'La prévention des risques liés aux chutes de hauteur a été évaluée.', 1, NULL, NULL),
(373, 128, 'La prévention des risques liés aux chutes de hauteur n\'a pas été évaluée.', 2, NULL, NULL),
(374, 129, 'CONFORMITÉ:\r\nLa plan de travail en hauteur ne dispose pas d’escalier d’accès sécurisé.', 2, NULL, NULL),
(375, 129, 'CONFORMITÉ:\r\nLa plan de travail en hauteur dispose d’un escalier d’accès sécurisé.\r\n', 1, NULL, NULL),
(376, 129, 'DÉGAGEMENT:\r\nAucun objet, marchandise ou matériel ne fait obstacle à la circulation des personnes.', 1, NULL, NULL),
(377, 129, 'MAIN COURANTE:\r\nLe ou les escaliers sont munis de rampe ou de main-courante.', 1, NULL, NULL),
(378, 129, 'DÉGAGEMENT:\r\nDes objets dans l’escalier, font obstacle à la circulation des personnes.', 2, NULL, NULL),
(379, 130, 'Le ou les plans de travail situés en hauteur ne disposent pas de garde-corps.', 2, NULL, NULL),
(380, 130, 'Le ou les plans de travail situés en hauteur disposent de garde-corps réglementaires.', 1, NULL, NULL),
(382, 130, 'Le ou les plans de travail situés en hauteur ne disposent pas de garde-corps réglementaires.', 2, NULL, NULL),
(385, 131, 'La structure ne dispose pas de plate-forme élévatrice mobile de personne (PEMP) mais il a été indiqué qu’il lui arrivé d’en louer occasionnellement', 1, NULL, NULL),
(386, 131, 'FORMATION CONDUITE:\r\nIl a été indiqué que, au moins deux agents disposent du CACES PEMP et d’une autorisation de conduite.', 1, NULL, NULL),
(388, 131, 'E.P.I:\r\nIl a été indiqué que les agents sont dotés des EPI nécessaires (harnais, longe, casque) et qu’ils les portent.', 1, NULL, NULL),
(389, 131, 'FORMATION E.P.I:\r\nIl a été indiqué que agents sont formés à l\'utilisation et à leur mise en oeuvre des Equipements de Protection Individuelle.', 1, NULL, NULL),
(390, 131, 'FORMATION CONDUITE:\r\nIl a été indiqué qu\'aucun agent n\'était formé à l\'utilisation et à leur mise en oeuvre d\'une plate forme élévatrice de personnel (PEMP)', 2, NULL, NULL),
(391, 131, 'FORMATION E.P.I:\r\nLes agents ont a leur disposition des harnais mais n’ont pas été formés à leur utilisation.', 2, NULL, NULL),
(392, 131, 'NOMBRE D\'AGENTS:\r\nIl a été indiqué que les interventions avec la PEMP ne se font pas systématiquement à deux agents.', 2, NULL, NULL),
(393, 132, 'EPI:\r\nLes agents disposent de harnais et de longes contre les chutes de hauteur.', 1, NULL, NULL),
(394, 132, 'FORMATION:\r\nIl a été indiqué que les agents ont été formés à leur utilisation.', 1, NULL, NULL),
(395, 132, 'FORMATION:\r\nIl a été indiqué que les agents n’ont pas été formés à l\'utilisation des dispositifs individuels anti-chute.', 2, NULL, NULL),
(397, 133, 'FORMATION:\r\nIl n’y a pas d’agent formé au montage, démontage de ce matériel.', 2, NULL, NULL),
(398, 133, 'FORMATION:\r\nIl a été indiqué qu’un agent est formé au montage et démontage de ce matériel.', 1, NULL, NULL),
(399, 133, 'NOTICE:\r\nLa notice de montage est disponible.', 1, NULL, NULL),
(400, 133, 'NOTICE:\r\nLa notice de montage n’est pas disponible.', 2, NULL, NULL),
(401, 133, 'ETAT GÉNÉRAL:\r\nLes différents éléments de l’échafaudage sont en bon état visuel.', 1, NULL, NULL),
(402, 133, 'VÉRIFICATION PÉRIODIQUE:\r\nLes vérifications trimestrielles sont effectuées et consignées.', 1, NULL, NULL),
(403, 133, 'VÉRIFICATION PÉRIODIQUE:\r\nLes vérifications périodiques ne sont pas effectuées et consignées.', 2, NULL, NULL),
(404, 134, 'ETAT:\r\nLa ou les échelles utilisées par les agents et vues lors de la visite, sont en bon état.', 1, NULL, NULL),
(405, 134, 'ETAT:\r\nToutes les échelles ne sont pas en bon état (patin anti-dérapant, déformation...)', 2, NULL, NULL),
(407, 134, 'UTILISATION ECHELLE:\r\nll a été indiqué que la ou les échelles ne sont utilisées comme poste de travail que de façon occasionnelle.', 1, NULL, NULL),
(408, 134, 'UTILISATION ECHELLE:\r\nll a été indiqué que la ou les échelles sont utilisées comme poste de travail.', 2, NULL, NULL),
(412, 135, 'Aucun inventaire des manutentions manuelles n’a été initié.', 2, NULL, NULL),
(413, 135, 'L’ensemble des manutentions manuelles ont été répertoriées.', 1, NULL, NULL),
(414, 135, 'Un plan d’action à été initié.', 1, NULL, NULL),
(415, 136, 'Des dispositifs d\'aide à la manutention sont disponibles et en nombre suffisant.', 1, NULL, NULL),
(416, 136, 'Absence de dispositif d\'aide à la manutention.', 2, NULL, NULL),
(417, 137, 'Les agents n’ont pas suivi de formation adéquate à la sécurité relative à l’exécution de ces opérations.', 2, NULL, NULL),
(418, 137, 'Les agents ont suivi une formation adéquate à la sécurité relative à l’exécution de ces opérations.', 1, NULL, NULL),
(419, 137, 'Les agents n’ont pas tous suivi une formation adéquate à la sécurité relative à l’exécution de ces opérations.', 2, NULL, NULL),
(420, 138, 'Les limites réglementaires de port de charges sont respectées.', 1, NULL, NULL),
(421, 138, 'Il a été indiqué que les limites réglementaires de port de charges ne sont pas respectées.', 2, NULL, NULL),
(422, 139, 'Lors de la visite, il a été indiqué que les agents communaux amenés à conduire les différents véhicules sont titulaires des permis réglementaires', 1, NULL, NULL),
(424, 140, 'Les agents ont suivi la formation FIMO et son à jour de leur recyclage (FCO).', 1, NULL, NULL),
(425, 140, 'Les agents n\'ont pas suivi la formation FIMO.', 2, NULL, NULL),
(426, 141, 'Les zones à risque ne sont pas signalées.', 2, NULL, NULL),
(427, 141, 'Les zones à risque sont signalées.', 1, NULL, NULL),
(428, 141, 'La signalisation applicable aux trafics est insuffisante.', 2, NULL, NULL),
(429, 142, 'MISE A DISPOSITION:\r\nLes agents  intervenant à pied sur le domaine routier à l’occasion d’un chantier ou d’un danger temporaire sont dotés de vêtements de signalisation classe 2 minimum.', 1, NULL, NULL),
(430, 142, 'PORT OBLIGATOIRE:\r\nIl a été indiqué que ces équipements sont portés.', 1, NULL, NULL),
(433, 142, 'ETAT:\r\nLes vêtements sont dans un état de propreté et d\'usure suffisant pour les maintenir efficaces.', 1, NULL, NULL),
(434, 142, 'MISE A DISPOSITION:\r\nLes agents  intervenant à pied sur le domaine routier à l’occasion d’un chantier ou d’un danger temporaire ne sont pas dotés de vêtements de signalisation classe 2 minimum.', 2, NULL, NULL),
(435, 143, 'SIGNALISATION:\r\nLe ou les véhicules ou engins qui par nécessité de service progressent lentement ou stationnent sur les chaussées sont équipés de la signalisation réglementaire.', 1, NULL, NULL),
(436, 143, 'SIGNALISATION:\r\nLe ou les véhicules qui par nécessité de service progressent lentement ou stationnent sur les chaussées ne sont pas équipés de la signalisation réglementaire. ', 2, NULL, NULL),
(437, 143, 'TRACTEUR EPAREUSE:\r\nLe tracteur épareuse utilisé lors d’un chantier mobile porte un AK5 muni de triflash.', 1, NULL, NULL),
(438, 143, 'TRACTEUR EPAREUSE:\r\nLe tracteur épareuse utilisé lors d’un chantier mobile n’est pas équipé de triflash.', 2, NULL, NULL),
(439, 143, 'OUTILS ATTELÉS:\r\nLes outils attelés ne sont pas signalés par des dispositifs rétroréfléchissants.', 2, NULL, NULL),
(440, 143, 'OUTILLAGES PORTES:\r\nLes outillages portés compris entre 1m et 4m inclus ne sont pas signalés par un dispositif face à l’arrière et deux disposés latéralement. (panneau carré 423x423 ou rectangulaire 423x282)', 2, NULL, NULL),
(441, 144, 'STOCK:\r\nLe centre technique dispose d’un stock  de panneaux de signalisation et il a été indiqué que les agents les utilisent lors de tous les chantiers temporaires.', 1, NULL, NULL),
(442, 144, 'STOCK:\r\nLe centre technique ne dispose pas d’un stock de panneaux de signalisation en nombre suffisant.', 2, NULL, NULL),
(443, 144, 'FIN DE PRESCRIPTION:\r\nPas de signalisation de fin de prescription.', 2, NULL, NULL),
(444, 144, 'ETAT:\r\nLes panneaux de signalisation sont en bon état.', 1, NULL, NULL),
(445, 144, 'ETAT:\r\nCertains panneaux sont usagés et détériorés, ce qui réduit passablement leur efficacité.', 2, NULL, NULL),
(446, 145, 'Il a été indiqué que les agents ont suivi une formation théorique au balisage de chantier lors de la formation CACES.', 1, NULL, NULL),
(447, 145, 'Il a été indiqué que les agents du service technique n’ont pas suivi de formation pratique s’agissant des risques liés aux interventions sur le domaine routier ( le balisage temporaire).', 2, NULL, NULL),
(448, 146, 'Le véhicule n’est pas équipé d’une séparation pouvant protéger des déplacements intempestifs des charges transportées.', 2, NULL, NULL),
(449, 146, 'Le véhicule est équipé d’une séparation pouvant protéger des déplacements intempestifs des charges transportées.', 1, NULL, NULL),
(450, 147, 'Le véhicule ne dispose pas de dispositif d’arrimage adapté aux charges à transporter.', 2, NULL, NULL),
(451, 147, 'Le véhicule dispose de dispositifs d’arrimage adaptés aux charges à transporter.', 1, NULL, NULL),
(452, 148, 'Le véhicule n’est pas adapté aux tâches à réaliser.', 2, NULL, NULL),
(453, 148, 'Le véhicule est adapté aux tâches à réaliser.', 1, NULL, NULL),
(458, 150, 'Le matériel dispose du marquage de conformité CE.', 1, NULL, NULL),
(459, 150, 'Matériel avec absence de marquage de conformité CE.', 2, NULL, NULL),
(460, 151, 'La machine n’a pas été contrôlé conformémént à la règlementation', 2, NULL, NULL),
(461, 151, 'La machine a été contrôlé conformément à la réglementation.', 1, NULL, NULL),
(462, 152, 'Matériel avec absence de marquage de conformité CE', 2, NULL, NULL),
(463, 152, 'Matériel modifié remettant en cause sa conformité CE', 2, NULL, NULL),
(464, 152, 'Le matériel dispose du marquage de conformité CE', 1, NULL, NULL),
(465, 153, 'Notice d’instruction disponible', 1, NULL, NULL),
(466, 153, 'Notice d’instruction indisponible.', 2, NULL, NULL),
(467, 154, 'Une information a été faite aux agents de maintenance des machine, un cahier de suivi à été mis en place', 1, NULL, NULL),
(468, 154, 'Aucune gestion de la maintenance des machines', 2, NULL, NULL),
(469, 155, 'Organe de sécurité en état.', 1, NULL, NULL),
(470, 155, 'Organe de sécurité détérioré.', 2, NULL, NULL),
(471, 156, 'L’engin est conforme à la réglementation.', 1, NULL, NULL),
(472, 156, 'L’engin n’est pas conforme à la réglementation.', 2, NULL, NULL),
(473, 157, 'TRACTEUR EPAREUSE:\r\nLe tracteur épareuse utilisé lors des chantiers mobiles portent un panneau AK5 muni de triflash.', 1, NULL, NULL),
(474, 157, 'TRACTEUR EPAREUSE:\r\nLe tracteur épareuse utilisé lors des chantiers mobiles n\'est pas équipé d\'un panneau AK5 muni de triflash.', 2, NULL, NULL),
(475, 158, 'Les VGP (Vérification Générale Périodique) des engins ont été réalisées.', 1, NULL, NULL),
(476, 158, 'Les VGP (Vérification Générale Périodique) des engins n’ont pas été réalisées.', 2, NULL, NULL),
(477, 158, 'Les VGP (Vérification Générale Périodique) de certains engins n’ont pas été réalisées.', 2, NULL, NULL),
(478, 159, 'La VGP (Vérification Générale Périodique) de la cuve du pulvérisateur a été effectuée.', 1, NULL, NULL),
(479, 159, 'La VGP (Vérification Générale Périodique) de la cuve du pulvérisateur n’a pas été effectuée.', 2, NULL, NULL),
(480, 160, 'Le ou les agents qui utilisent la tondeuse autoportée ont suivis une formation spécifique à la conduite en sécurité.', 1, NULL, NULL),
(481, 160, 'Le ou les agents qui utilisent la tondeuse autoportée n\'ont pas suivis de formation spécifique à la conduite en sécurité pour ce type d\'engin.', 2, NULL, NULL),
(482, 161, 'La ou les tondeuses autoportées sont équipées de l\'éclairage sécurité réglementaire. (Gyrophare, éclairage de position et de changement de direction)', 1, NULL, NULL),
(483, 161, 'La ou les tondeuses autoportées ne sont pas équipées de l\'éclairage sécurité réglementaire. (Gyrophare et/ou éclairage de position et/ou de changement de direction)', 2, NULL, NULL),
(484, 162, 'La tondeuse autoportée est susceptible d’évoluer sur des sols en forte pente, elle est équipée d’un système anti-écrasement.', 1, NULL, NULL);
INSERT INTO `observation` (`NUM_OBSERVATION`, `NUM_CRITERE`, `LIBELLE_OBSERVATION`, `CODE_COULEUR_OBSERVATION`, `code_obs`, `DATE_OBSERVATION`) VALUES
(485, 162, 'La tondeuse autoportée est susceptible d’évoluer sur des sols en forte pente et n’est pas équipé d’un système anti-écrasement.', 2, NULL, NULL),
(486, 163, 'La tondeuse auto-portée est homologuée pour circuler sur la voie publique.', 1, NULL, NULL),
(487, 163, 'La tondeuse auto-portée est amenée à circuler sur la voie publique mais n’est pas homologuée et équipée de la signalisation reglementaire.', 2, NULL, NULL),
(488, 164, 'Il a été indiqué que l\'Engin de Service Hivernal a fait l\'objet d\'une exemption de "réception à titre isolé" auprès des services de la DREAL.', 1, NULL, NULL),
(489, 164, 'Il a été indiqué que l\'Engin de Service Hivernal n\'a pas fait l\'objet d\'une "réception à titre isolé" auprès des services de la DREAL.', 2, NULL, NULL),
(490, 165, 'L’éclairage et la signalisation sont conformes.', 1, NULL, NULL),
(491, 165, 'L’éclairage et la signalisation ne sont pas conformes.', 2, NULL, NULL),
(492, 166, 'Le ou les agents sont titulaires du permis correspondant à la catégorie du véhicule et on suivi une formation à la conduite en sécurité.', 1, NULL, NULL),
(493, 166, 'Le ou les agents ne sont pas titulaires du permis corespondant à la catégorie du véhicule', 2, NULL, NULL),
(494, 166, 'Le ou les agents n’ont pas suivi de formation adapté corespondante au type de véhicule', 2, NULL, NULL),
(496, 167, 'La lame fournie à l\'exploitant agricole est en conformité.', 1, NULL, NULL),
(497, 167, 'La lame fournie à l\'exploitant agricole n\'est pas en conformité.', 2, NULL, NULL),
(498, 168, 'EVALUATION DES RISQUES:\r\nLe risque chimique a fait l\'objet d\'une évaluation des risques dans le document unique.', 1, NULL, NULL),
(499, 168, 'EVALUATION DES RISQUES:\r\nLe risque chimique n\'a pas fait l\'objet d\'une évaluation des risques dans le document unique.', 2, NULL, NULL),
(501, 169, 'Il a été indiqué que certains agents du service technique ont suivi la formation certifiante ’Certi-phyto’', 1, NULL, NULL),
(502, 169, 'Il a été indiqué que les agents du service technique n’ont pas suivi de formation à l’utilisation des produits dangereux.', 2, NULL, NULL),
(503, 169, 'Il a été indiqué que des agents du restaurant scolaire ont suivi une formation sur cette problématique dans le cadre de l’HACCP.', 1, NULL, NULL),
(504, 169, 'Il a été indiqué que les agents d’entretien n’ont pas suivi de formation à l’utilisation des produits dangereux.', 2, NULL, NULL),
(505, 170, 'Il a été indiqué que les FDS (Fiches de données de sécurité) des produits utilisés et stockés ont été recensées et sont mises à disposition sur les sites de stockage.', 1, NULL, NULL),
(506, 170, 'INVENTAIRE:\r\nIl a été indiqué que les FDS (Fiches de données de sécurité) des produits utilisés et stockés ne sont pas recensées.', 2, NULL, NULL),
(507, 171, 'La liste des travailleurs exposés a été communiquée au service de médecine préventive.', 1, NULL, NULL),
(508, 171, 'La liste des travailleurs exposés n\'a pas été communiquée au service de médecine préventive.\r\n', 2, NULL, NULL),
(509, 172, 'Une démarche de substitution à été initié.', 1, NULL, NULL),
(510, 172, 'Aucune démarche de substitution n’a été initié.', 2, NULL, NULL),
(511, 173, 'Les notices de poste (consignes d\'utilisation et de secours) sont affichées sur le lieu de manipulation.', 1, NULL, NULL),
(512, 173, 'Les notices de postes (consignes d\'utilisation et de secours) sont incomplètes.', 2, NULL, NULL),
(513, 173, 'Absence de notice de poste (consigne d\'utilisation et de secours) sur le lieu de manipulation.', 2, NULL, NULL),
(514, 174, 'Le dispositif de captage des polluants est suffisant.', 1, NULL, NULL),
(515, 174, 'Le dispositif de captage des polluants est insuffisant.', 2, NULL, NULL),
(516, 175, 'Il a été indiqué que les agents utilisant les produits sont dotés d’équipements de protection (gants, vêtements, chaussures)', 1, NULL, NULL),
(517, 175, 'Il Ia été indiqué que les agents assurant l’entretien des locaux ne sont pas dotés de l’ensemble des équipements de protection nécessaire (pas de gant, de chaussures, de vêtement...)', 2, NULL, NULL),
(518, 175, 'Il a été indiqué que des équipements individuels tels que combinaisons,gants, bottes, lunettes, masques ont été mis à disposition des agents.', 1, NULL, NULL),
(519, 175, 'Il a été indiqué que tous les équipements individuels nécessaires ne sont pas fournis (pas de combinaisons,gants, bottes, masques, lunettes )', 2, NULL, NULL),
(520, 175, 'Pas de gant de protection chimique et de gant de protection contre la coupure (gant textile en Kévlar à porter sous un gant d’hygiène) à disposition.', 2, NULL, NULL),
(521, 175, 'Privilégier les chaussures fermées pour les activités de restauration nécessitant de multiples déplacements', 2, NULL, NULL),
(522, 176, 'STOCKAGE:\r\nLes produits chimiques sont stockés dans un local (ou une armoire réservé à cet usage), ventilé, fermé à clé, clairement identifié et pourvu d’un dispositif de rétention spécifique.', 1, NULL, NULL),
(523, 176, 'LOCAL:\r\nLes produits d’entretien sont stockés dans un local réservé à cet usage mais sans aération, pas fermé à clé et dépourvu de dispositif de rétention spécifique.', 2, NULL, NULL),
(524, 176, 'ARMOIRE:\r\nLes produits d’entretien sont stockés dans une armoire sans distinction, mélangés à des consommables, sans aération, pas fermé à clé et dépourvu de dispositif de rétention spécifique.', 2, NULL, NULL),
(526, 176, 'STOCKAGE:\r\nLe stockage des produits chimiques n\'est pas conforme à la réglementation.', 2, NULL, NULL),
(527, 177, 'La liste des travailleurs exposés a été communiquée au service de médecine préventive.', 1, NULL, NULL),
(528, 177, 'La liste des travailleurs exposés n\'a pas été communiquée au service de médecine préventive.', 2, NULL, NULL),
(529, 178, 'Il a été indiqué que la collectivité dispose d\'agents formés et habilités à intervenir sur les installations électriques.', 1, NULL, NULL),
(530, 178, 'Il a été indiqué que la collectivité ne dispose pas d\'agent formé et habilité à intervenir sur les installations électriques.', 2, NULL, NULL),
(531, 179, 'La ou les installations sont conformes (rapport de vérification périodique sans remarque)', 1, NULL, NULL),
(533, 180, 'La ou les armoires sont signalées par un pictogramme de danger électrique.', 1, NULL, NULL),
(534, 180, 'La ou les armoires ne sont pas signalées par un pictogramme de danger électrique.', 2, NULL, NULL),
(535, 181, 'EVALUATION RISQUE BRUIT: \r\nLe risque bruit a fait l\'objet d\'une évaluation des risques dans le document unique.', 1, NULL, NULL),
(536, 181, 'EVALUATION RISQUE BRUIT:\r\nLe risque bruit n\'a pas fait l\'objet d\'une évaluation des risques dans le document unique.', 2, NULL, NULL),
(537, 182, 'MISE A DISPOSITION:\r\nDes équipements de protection individuelles contre les nuisances sonores sont à disposition des agents.', 1, NULL, NULL),
(538, 182, 'MISE A DISPOSITION:\r\nIl n\'y a pas d\'équipement de protection individuelle contre les nuisances sonores à disposition des agents.', 2, NULL, NULL),
...
(540, 183, 'FORMATION RISQUE BRUIT:\r\nIl a été indiqué que les agents n\'ont reçu aucune formation/information sur les risques liés au bruit', 2, NULL, NULL),
(541, 184, 'SUIVI MEDICAL:\r\nLes bruits auxquels sont exposés les agents ne dépassent pas les valeurs d\'exposition', 1, NULL, NULL),
(542, 184, 'SUIVI MEDICAL:\r\nIl a été indiqué que les agents exposés n\'ont pas eu d\'examen audiométrique préventif ', 2, NULL, NULL),
(543, 185, 'MESURES D\'EXPOSITION:\r\nDes mesures d\'exposition ont été réalisées  et ne font apparaître aucun dépassement des valeurs limites', 1, NULL, NULL),
(544, 185, 'MESURES D\'EXPOSITION:\r\nIl a été indiqué qu\'aucune mesure de niveaux de bruit n\'a été réalisée', 2, NULL, NULL),
(545, 186, 'EVALUATION DES RISQUES:\r\nLe risque biologique a fait l\'objet d\'une évaluation des risques dans le document unique (DUERP)', 1, NULL, NULL),
(546, 186, 'EVALUATION DES RISQUES:\r\nAucune mesures n’a été prise pour réduire l’exposition aux agents biologiques dangereux', 2, NULL, NULL),
(547, 186, 'EVALUATION DES RISQUES:\r\nLes mesures prise pour réduire l’exposition aux agents biologiques dangereux sont insuffisantes', 2, NULL, NULL),
(548, 187, 'Une démarche de suppression de l\'exposition au risque a été initiée.', 1, NULL, NULL),
(549, 187, 'Aucune démarche de suppression de l\'exposition au risque n\'a été initiée.', 2, NULL, NULL),
...
(551, 188, 'LISTE AGENTS EXPOSES:\r\nAucune information sur les agents exposés aux risques biologiques n\'a été transmise au médecin de prévention.', 2, NULL, NULL),
(552, 189, 'INSTRUCTION:\r\nIl existe une instruction écrite portant sur la procédure à suivre en cas d’accident ou d’incident grave mettant en cause un agent biologique.', 1, NULL, NULL),
(553, 189, 'INSTRUCTION:\r\nIl n\'existe pas d\'instruction écrite portant sur la procédure à suivre en cas d’accident ou d’incident grave mettant en cause un agent biologique.', 2, NULL, NULL),
(554, 190, 'Le risque lié aux vibrations mécaniques a fait l\'objet d\'une évaluation des risques dans le document unique (DUERP)', 1, NULL, NULL),
(555, 190, 'Le risque lié aux vibrations mécaniques n\'a pas fait l\'objet d\'une évaluation des risques dans le document unique.', 2, NULL, NULL),
(556, 191, 'MESURES D\'EXPOSITION:\r\nDes mesures d\'exposition ont été réalisées  et ne font apparaître aucun dépassement des valeurs limites.', 1, NULL, NULL),
(557, 191, 'MESURES D\'EXPOSITION:\r\nIl a été indiqué qu\'aucune évaluation ou mesure des niveaux de vibration mécanique n\'a été réalisée.', 2, NULL, NULL),
(558, 192, 'SUIVI MÉDICAL:\r\nLes agents exposés aux vibrations mécaniques dépassant les valeurs limites d\'exposition sont suivi par le médecin de prévention.', 1, NULL, NULL),
(559, 192, 'SUIVI MÉDICAL:\r\nLe médecin de prévention n\'a pas été informé des résultats de l\'évaluation des niveaux de vibrations mécaniques ou du mesurage.', 2, NULL, NULL),
(560, 193, 'L’implantation du bureau est adaptée.', 1, NULL, NULL),
(561, 193, 'L’implantation du bureau est à revoir.', 2, NULL, NULL),
(562, 194, 'L’éclairage est suffisant et les protections contre l’éblouissement sont présentes.', 1, NULL, NULL),
(563, 194, 'L’éclairage est insuffisant et les protections contre l’éblouissement sont absentes.', 2, NULL, NULL),
(564, 195, 'MATÉRIEL INFORMATIQUE ET MOBILIER:\r\nLe bureau est équipé de matériel informatique et de mobilier adapté.', 1, NULL, NULL),
(565, 195, 'MATÉRIEL INFORMATIQUE ET MOBILIER:\r\nLe bureau n’est pas équipé de matériel informatique et de mobilier adapté.', 2, NULL, NULL),
(566, 196, 'Une organisation du travail est mise en place pour limiter l’astreinte visuelle et la fatigue posturale.', 1, NULL, NULL),
(567, 196, 'Aucune organisation de travail n’a été mise en place pour limiter l’astreinte visuelle et la fatigue posturale.', 2, NULL, NULL),
(570, 197, 'La surface du bureau et les largeurs de passage correspondent à la norme.', 1, NULL, NULL),
(571, 197, 'La surface du bureau et les largeurs de passage sont inférieures à la norme.', 2, NULL, NULL),
(572, 198, 'Il existe une procédure pour l\'élaboration d\'un protocole sécurité (chargement/déchargement) avec les fournisseurs/livreurs.', 1, NULL, NULL),
(573, 198, 'Il n\'existe pas de procédure pour l\'élaboration d\'un protocole sécurité (chargement/déchargement) avec les fournisseurs/livreurs.', 2, NULL, NULL),
(574, 199, 'Il existe une procédure mise en place pour l\'accueil d\'entreprises extérieures et la rédaction de plan de prévention.', 1, NULL, NULL),
(575, 199, 'Il n\'existe pas de procédure mise en place pour l\'accueil d\'entreprises extérieures et la rédaction de plan de prévention.', 2, NULL, NULL),
(576, 200, 'Des règles de sécurité et d\'intervention ont été planifiées.', 1, NULL, NULL),
(577, 200, 'Les règles de sécurité et d\'intervention n\'ont pas été planifiées.', 2, NULL, NULL),
(578, 201, 'Le ou les agents sont équipés des EPI nécessaires.', 1, NULL, NULL),
(579, 201, 'Le ou les agents ne sont pas équipés de tous les EPI nécessaires.', 2, NULL, NULL),
(580, 202, 'Le ou les agents en situation de travailleurs isolés disposent d\'un moyen de communication.', 1, NULL, NULL),
(581, 202, 'Le ou les agents en situation de travailleurs isolés ne disposent pas d\'un moyen de communication professionnel.', 2, NULL, NULL),
(583, 203, 'Le ou les agents ont suivi la formation obligatoire.', 1, NULL, NULL),
(584, 203, 'Le ou les agents n’ont pas suivi la formation obligatoire.', 2, NULL, NULL),
(585, 204, 'La commune est habilitée par la préfecture.', 1, NULL, NULL),
(586, 204, 'La commune n’est pas habilitée par la préfecture.', 2, NULL, NULL),
(587, 205, 'Des règles de sécurité formalisées ont été soumises aux agents.', 1, NULL, NULL),
(588, 205, 'Aucune règle de sécurité n\'a été formalisée et portée à la connaissance des agents.', 2, NULL, NULL),
(589, 206, 'Le ou les plans de travail situés en hauteur disposent de garde-corps réglementaires.', 1, NULL, NULL),
(590, 206, 'Le ou les plans de travail situés en hauteur ne disposent pas de garde-corps.', 2, NULL, NULL),
(591, 207, 'Le ou les agents en situation de travailleurs isolés disposent d\'un moyen de communication.', 1, NULL, NULL),
(592, 207, 'Le ou les agents en situation de travailleurs isolés ne disposent pas d\'un moyen de communication professionnel.', 2, NULL, NULL),
(593, 208, 'Les règles de sécurité formalisées ont été soumises aux agents.', 1, NULL, NULL),
(594, 208, 'Aucune règle de sécurité n\'a été formalisée et portée à la connaissance des agents.', 2, NULL, NULL),
(595, 209, 'Le ou les agents sont équipés des EPI adaptés et nécessaires.', 1, NULL, NULL),
(596, 209, 'Le ou les agents ne sont pas équipés de tous les EPI nécessaires.', 2, NULL, NULL),
(597, 210, 'Le ou les agents en situation de travailleurs isolés disposent d\'un moyen de communication.', 1, NULL, NULL),
(598, 210, 'Le ou les agents en situation de travailleurs isolés ne disposent pas d\'un moyen de communication professionnel.', 2, NULL, NULL),
(599, 211, 'Les agents disposent d\'un système de captage des fumées.', 1, NULL, NULL),
(600, 211, 'Les agents ne disposent pas d\'un système de captage des fumées.', 2, NULL, NULL),
(601, 212, 'Le ou les agents qui utilisent des produits phyto-sanitaires ont suivi la formation Certi-Phyto. (applicateur et applicateur opérationnel).', 1, NULL, NULL),
(602, 212, 'Le ou les agents qui utilisent des produits phyto-sanitaires n\'ont pas suivi la formation Certi-Phyto. ', 2, NULL, NULL),
(603, 213, 'Les EPI sont stockés dans un local ou une armoire différente du lieu de stockage des produits phytosanitaires.', 1, NULL, NULL),
(604, 213, 'Les EPI ne sont pas stockés dans un local ou une armoire différente du lieu de stockage des produits phytosanitaires.', 2, NULL, NULL),
(605, 214, 'Présence d\'un registre des applications des produits phytosanitaires.', 1, NULL, NULL),
(606, 214, 'Absence de registre des applications des produits phytosanitaires.', 2, NULL, NULL),
(607, 215, 'Les produits Phytosanitaires sont stockés dans une armoire ou un local spécifique.', 1, NULL, NULL),
(608, 215, 'Les produits Phytosanitaires ne sont pas stockés dans une armoire ou un local spécifique.', 2, NULL, NULL),
(609, 216, 'Les agents intervenants sur la fabrication ou la manutention des repas ont suivi la formation réglementaire à l\'hygiène alimentaire (HACCP)', 1, NULL, NULL),
(610, 216, 'Les agents intervenants sur la fabrication ou la manutention des repas n\'ont pas suivi de formation  à l\'hygiène alimentaire (HACCP)', 2, NULL, NULL),
(611, 217, 'La collectivité  a effectué une déclaration d\'activité alimentaire auprès des services de la DDCPP', 1, NULL, NULL),
(612, 217, 'La collectivité  n\'a pas effectué de déclaration d\'activité alimentaire auprès des services de la DDCPP', 2, NULL, NULL),
(613, 218, 'L\'aménagement du ou des postes de travail est adapté (matériels et équipements de travail conformes), les principes d\'ergonomie (chariots adaptés, plans de travail à hauteur recommandée...) sont respectés.', 1, NULL, NULL),
(614, 218, 'L\'aménagement du ou des postes de travail est à revoir.', 2, NULL, NULL),
(615, 219, 'Les agents sont formés aux différents risques relatifs au travail en piscine.', 1, NULL, NULL),
(616, 219, 'Les agents ne sont pas formés aux différents risques relatifs au travail en piscine (manutentions, manipulation des produits chimiques, secourisme, nage...).', 2, NULL, NULL),
(617, 220, 'Il a été indiqué qu\'une organisation est mise en place afin d\'éviter qu\'un agent ne se trouve en situation de travailleur isolé lorsqu\'il effectue des travaux dangereux (nettoyage en bordure de bassin, manipulation de produits dangereux...)', 1, NULL, NULL),
(618, 220, 'Aucune organisation n\'est mise en place afin d\'éviter qu\'un agent ne se trouve en situation de travailleur isolé lorsqu\'il effectue des travaux dangereux (nettoyage en bordure de bassin, manipulation de produits dangereux...)', 2, NULL, NULL),
(619, 221, 'Le ou les agents de l\'atelier menuiserie ont suivi une formation spécifique liée à leur activité.', 1, NULL, NULL),
(620, 221, 'Le ou les agents de l\'atelier menuiserie n\'ont pas suivi de formation spécifique liée à leur activité.', 2, NULL, NULL),
(621, 222, 'Présence de système d\'aspiration des poussières de bois.', 1, NULL, NULL),
(622, 222, 'Absence de système d\'aspiration des poussières de bois.', 2, NULL, NULL),
(623, 223, 'Des mesures de VLEP ont été effectuées.', 1, NULL, NULL),
(624, 223, 'Aucune mesure de VLEP n\'a été effectuée.', 2, NULL, NULL),
(625, 224, 'L\'accessibilité des locaux de travail a été prise en compte pour le ou les agents travailleurs handicapés.', 1, NULL, NULL),
(626, 224, 'L\'accessibilité des locaux de travail n\'a pas été prise en compte pour le ou les agents travailleurs handicapés.', 2, NULL, NULL),
(627, 225, 'Les opérations d\'entretien et de vérification sont effectuées et formalisées.', 1, NULL, NULL),
(628, 225, 'Les opérations de vérification et d\'entretien ne sont pas toutes effectuées et formalisées', 2, NULL, NULL),
(629, 226, 'La vérification des aires de jeux est effectuée.', 1, NULL, NULL),
(630, 226, 'La vérification des aires de jeux n\'est pas effectuée.', 2, NULL, NULL),
(631, 227, 'AMIANTE:\r\nLes DTA ont été faits et les résultats ont été communiqués à la médecine préventive.', 1, NULL, NULL),
(632, 227, 'AMIANTE:\r\nLes DTA n\'ont pas été faits sur tous les bâtiments.', 2, NULL, NULL),
(643, 229, 'DIAGNOSTIC RADON:\r\nUn diagnostic radon a été réalisé dans les locaux', 1, NULL, NULL),
(644, 229, 'DIAGNOSTIC RADON:\r\nAucune mesures ', 2, NULL, NULL),
(649, 230, 'Il a été indiqué que le ou les arbres à cardan n\'ont pas fait l\'objet d\'une vérification périodique (annuelle)', 2, NULL, NULL),
(651, 53, 'Il a été indiqué que toutes les installations électriques n\'ont pas été vérifiées par un organisme agréé ou une personne qualifiée .', 2, NULL, NULL),
(654, 232, 'Des chaussures de sécurité anti-dérapantes à embout renforcé sont à disposition.', 1, NULL, NULL),
(655, 232, 'Absence de chaussure de sécurité à embout renforcé.', 2, NULL, NULL),
(656, 97, 'VESTIAIRES:\r\nAbsence de vestiaire.', 2, NULL, NULL),
(658, 93, 'Une zone de danger observée lors de l\'inspection est à signaler par des dispositifs visuels', 2, NULL, NULL),
(659, 106, 'LOCAUX:\r\nTous les locaux où interviennent les agents sont  pourvus d\'une trousse de premiers secours ', 1, NULL, NULL),
(660, 106, 'VÉHICULES:\r\nLes véhicules de service ne sont pas équipés de trousse de secours', 2, NULL, NULL),
(661, 106, 'CONTENU:\r\nLe contenu de certaines trousses de secours est à revoir (produits manquants ou périmés)', 2, NULL, NULL),
(662, 107, 'Un affichage des numéros d\'urgence est présent, il est cependant nécessaire de le compléter', 2, NULL, NULL),
(663, 108, 'ASTREINTE:\r\nUne procédure est également en place pour les interventions sous astreinte.', 1, NULL, NULL),
(664, 108, 'ASTREINTE:\r\nAucune procédure de gestion du travail isolé n\'a été mise en place pour les interventions sous astreinte', 2, NULL, NULL),
(665, 109, 'TÉLÉPHONE PORTABLE:\r\nIl a été indiqué que les agents amenés à se déplacer dans des zones isolées, ont à disposition des téléphones portables professionnels.', 1, NULL, NULL),
(666, 109, 'TÉLÉPHONE PORTABLE:\r\nIl a été indiqué que les agents amenés à se déplacer dans des zones isolées, n\'ont pas à leur disposition de téléphone portable professionnel.', 2, NULL, NULL),
(667, 121, 'Le ou les escaliers d\'une largeur au moins égale à 1,5 mètre sont munis d\'une main courante de chaque côté.', 1, NULL, NULL),
(668, 121, 'Aucun objet, marchandise ou matériel ne fait obstacle à la circulation des personnes dans le ou les escaliers.', 1, NULL, NULL),
(669, 121, 'Le ou les escaliers d\'une largeur au moins égale à 1,5 mètre ne sont pas munis d\'une main courante de chaque côté.', 2, NULL, NULL),
(670, 122, 'Le bâtiment n\'est pas équipé d\'un système d\'alarme sonore.', 2, NULL, NULL),
(671, 123, 'Absence de Bloc Autonome d\'Eclairage Sécurité (BAES). ', 2, NULL, NULL),
(672, 125, 'La fosse de visite est conforme aux recommandation R468 et R469.', 1, NULL, NULL),
(673, 129, 'MAIN COURANTE:\r\nLe ou les escaliers ne sont pas munis de rampe ou de main-courante.', 2, NULL, NULL),
(674, 129, 'MAINS COURANTES:\r\nLe ou les escaliers d’une largeur au moins égale à 1,5 mètre ne sont pas munis de main courante de chaque côté.', 2, NULL, NULL),
(675, 132, 'EPI:\r\nLes agents ne disposent pas de harnais et de longe contre les chutes de hauteur.', 2, NULL, NULL),
(676, 132, 'CONTRÔLE:\r\nIl a été indiqué que le ou les harnais n\'ont pas été vérifiés réglementairement au cours des 12 derniers mois.', 2, NULL, NULL),
(677, 132, 'CONTRÔLE:\r\nIl a été indiqué que les harnais ont été contrôlés au cours des 12 derniers mois.', 1, NULL, NULL),
(678, 133, 'ETAT GÉNÉRAL:\r\nLa structure de l\'échafaudage présente des non conformités (déformation,\r\nprésence de corrosion, absence d\'élément de fixation...)', 2, NULL, NULL),
(681, 233, 'ETAT:\r\nLe ou les escabeaux utilisés par les agents et vus lors de la visite, sont en mauvais état.', 2, NULL, NULL),
(682, 233, 'ETAT:\r\nLe ou les escabeaux utilisés par les agents et vus lors de la visite, sont en bon état.', 1, NULL, NULL),
(683, 233, 'UTILISATION ESCABEAU:\r\nIl a été indiqué que les escabeaux sont utilisés pour des interventions occasionnelles et de courte durée.', 1, NULL, NULL),
(684, 131, 'NOMBRE D\'AGENTS FORMES:\r\nIl a été indiqué qu’un seul agent dispose du CACES PEMP et d’une autorisation de conduite or la présence d’un surveillant formé est nécessaire.', 2, NULL, NULL),
(685, 164, 'Il a été indiqué que l\'Engin de Service Hivernal a fait l\'objet d\'une "réception à titre isolé" auprès des services de la DREAL.', 1, NULL, NULL),
(686, 150, 'Matériel modifié remettant en cause sa conformité CE.', 2, NULL, NULL),
(687, 139, 'Il n\'y a aucun mesure de vérification de la validité du permis de conduire des agents.', 2, NULL, NULL),
(688, 140, 'Les agents ont suivi la formation FIMO mais ne sont  pas à jour de leur recyclage (FCO).', 2, NULL, NULL),
(689, 168, 'EVALUATION DES RISQUES:\r\nL\'évaluation des risques n\'a pas pris en compte l\'ensemble des activités des agents impliquant une exposition à des agents chimiques.', 2, NULL, NULL),
(690, 174, 'Pas de dispositif de captage des polluants.', 2, NULL, NULL),
(691, 178, 'Il a été indiqué que les agents formés ne sont pas à jour de leur recyclage (recommandation tous les 3 ans)', 2, NULL, NULL),
(692, 181, 'EVALUATION RISQUE BRUIT:\r\nL\'évaluation des risques n\'a pas pris en compte l\'ensemble des activités exposées au risque bruit.', 2, NULL, NULL),
(693, 182, 'ETAT:\r\nLes équipements  de protection individuelle à disposition des agents sont usagés ou détériorés.', 2, NULL, NULL),
(694, 182, 'ETAT:\r\nLes équipements  de protection individuelle à disposition des agents sont en bon état.', 1, NULL, NULL),
(695, 182, 'PORT:\r\nLes agents disposent d\'équipements de protection individuelles contre les nuisances sonores, mais il a été indiqué qu\'ils ne les portent pas systématiquement', 2, NULL, NULL),
(696, 184, 'SUIVI MEDICAL:\r\nLes agents exposés aux bruits dépassant les valeurs limites d\'exposition sont suivi par le médecin de prévention (examen audiométrique)', 1, NULL, NULL),
(697, 185, 'MOYENS DE REDUCTION:\r\nDes mesures d\'exposition ont fait apparaître des niveaux supérieurs aux valeurs limites mais l\'employeur a mis en oeuvre un programme de mesures correctives visant à réduire l\'exposition.', 1, NULL, NULL),
(698, 185, 'SIGNALISATION:\r\nLes lieux de travail où des niveaux de bruit dépassant les valeurs limites ont été mesurés, sont signalés', 1, NULL, NULL),
(699, 185, 'MOYENS DE REDUCTION:\r\nDes mesures d\'exposition ont fait apparaître des niveaux supérieurs aux valeurs limites et aucune mesure corrective visant à réduire l\'exposition n\'a été mise en place', 2, NULL, NULL),
(700, 185, 'SIGNALISATION:\r\nIl n\'existe aucune signalisation des endroits où les agents sont susceptibles d\'être exposés à  un bruit dépassant les valeurs supérieures d\'exposition (cf art.R4434-3 du CdT)', 2, NULL, NULL),
(701, 234, 'Les agents ont à disposition les EPI adaptés aux risques liés à leur activité.', 1, NULL, NULL),
(702, 234, 'Les agents n\'ont pas à leur disposition les EPI adaptés aux risques liés à leur activité.', 2, NULL, NULL),
(703, 235, 'Une organisation est mise en place pour la prévention des risques de chute et de noyade.', 1, NULL, NULL),
(704, 235, 'Aucune organisation n\'est mise en place pour la prévention des risques de chute et de noyade.', 2, NULL, NULL),
(705, 236, 'Des prélèvements d\'air sont effectués et la valeur limite n\'est pas dépassée.', 1, NULL, NULL),
(706, 236, 'Aucun contrôle concernant les trichloramines n\'est effectué.', 2, NULL, NULL),
(707, 234, 'Les agents n\'ont pas à disposition l\'ensemble des EPI adaptés aux risques liés à leur activité.', 2, NULL, NULL),
(708, 235, 'Les moyens techniques et organisationnels mis en place sont insuffisants pour prévenir les risques de chute et de noyade.', 2, NULL, NULL),
(709, 220, 'L\' organisation mise en place afin d\'éviter qu\'un agent ne se trouve en situation de travailleur isolé lorsqu\'il effectue des travaux dangereux (nettoyage en bordure de bassin, manipulation de produits dangereux...), est insuffisante.', 2, NULL, NULL),
(710, 193, 'L’aménagement du bureau est conforme.', 1, NULL, NULL),
(711, 193, 'L’aménagement du bureau est à optimiser.', 2, NULL, NULL),
(712, 193, 'L’implantation et l’aménagement du ou des bureaux sont à optimiser.', 2, NULL, NULL),
(713, 194, 'L’éclairage est suffisant.', 1, NULL, NULL),
(714, 194, 'Les protections contre l’éblouissement sont présentes.', 1, NULL, NULL),
(715, 194, 'L’éclairage est insuffisant.', 2, NULL, NULL),
(716, 194, 'Les protections contre l’éblouissement sont absentes.', 2, NULL, NULL),
(717, 195, 'MATÉRIEL INFORMATIQUE:\r\nLe bureau est équipé de matériel informatique adapté.', 1, NULL, NULL),
(718, 195, 'MATÉRIEL INFORMATIQUE:\r\nLe bureau n’est pas équipé de matériel informatique adapté.', 2, NULL, NULL),
(719, 195, 'MOBILIER:\r\nLe bureau est équipé de mobilier adapté.', 1, NULL, NULL),
(720, 195, 'MOBILIER:\r\nLe bureau n’est pas équipé de mobilier adapté.', 2, NULL, NULL),
(721, 197, 'SURFACE:\r\nLa surface du bureau correspond à la norme.', 1, NULL, NULL),
(722, 197, 'LARGEUR DE PASSAGE:\r\nLes largeurs de passage correspondent à la norme.', 1, NULL, NULL),
(723, 197, 'SURFACE:\r\nLa surface du bureau est inférieure à la norme.', 2, NULL, NULL),
(724, 197, 'LARGEUR DE PASSAGE:\r\nLa ou les largeurs de passage sont inférieures à la norme.', 2, NULL, NULL),
(725, 237, 'Les voies de circulation sont dégagées.', 1, NULL, NULL),
(726, 237, 'Présence de câbles électriques dans la ou les voies de circulation.', 2, NULL, NULL),
(727, 237, 'Présence d’objets dans la ou les voies de circulation.', 2, NULL, NULL),
(728, 236, 'Absence de produit ou de procédé qui permette de réduire la teneur en chlore combiné dans les bassins.', 2, NULL, NULL),
(729, 201, 'Le ou les agents ne sont pas équipés des EPI nécessaires.', 2, NULL, NULL),
(730, 204, 'L’habilitation date de plus de 6 ans et n’a pas été renouvelée.', 2, NULL, NULL),
(731, 190, 'Les risques liés aux vibrations mécaniques n\'ont pas été évalués sur tous les équipements de travail.', 2, NULL, NULL),
(732, 191, 'MOYENS DE REDUCTION:\r\nDes mesures d\'exposition ont fait apparaître des niveaux supérieurs aux valeurs limites mais l\'employeur a mis en oeuvre un programme de mesures correctives visant à réduire l\'exposition.', 1, NULL, NULL),
(734, 191, 'MOYENS DE RÉDUCTION:\r\nDes mesures d\'exposition ont fait apparaître des niveaux supérieurs aux valeurs limites et toutes les mesures correctives visant à réduire l\'exposition n\'ont pas été mises en place.', 2, NULL, NULL),
(735, 229, 'DIAGNOSTIC RADON:\r\nDes mesures ont été réalisées et ne font pas apparaître de dépassement des valeurs limites réglementaires', 1, NULL, NULL),
(736, 229, 'DIAGNOSTIC RADON:\r\nDes mesures ont été réalisées et font apparaître des dépassements des valeurs limites réglementaires', 2, NULL, NULL),
(737, 229, 'SUIVI MEDICAL:\r\nLes agents exposées aux rayonnements RADON  ne sont pas suivis par le médecin de prévention', 2, NULL, NULL),
(738, 229, 'SUIVI MEDICAL:\r\nLes agents exposés aux rayonnements RADON sont identifiés et suivi  par le médecin de prévention', 1, NULL, NULL),
(739, 229, 'PREVENTION:\r\nDes mesures de prévention (organisationnelles et techniques) sont mises en place', 1, NULL, NULL),
(740, 229, 'PREVENTION:\r\nAucune mesure prévue par la réglementation n\'a été mise en place', 2, NULL, NULL),
(741, 97, 'ARMOIRES:\r\nLes vestiaires sont équipés d’armoires individuelles (à double compartiment pour les travaux salissants).', 1, NULL, NULL),
(742, 105, 'Aération par ventilation insuffisante.', 2, NULL, NULL),
(743, 120, 'NOMBRE:\r\nLe nombre de dégagement est insuffisant.', 2, NULL, NULL),
(744, 121, 'Des objets dans l\'escalier, font obstacle à la circulation des personnes.', 2, NULL, NULL),
(745, 129, 'MAINS COURANTES:\r\nLe ou les escaliers d’une largeur au moins égale à 1,5 mètre sont munis de main courante de chaque côté.', 1, NULL, NULL),
(746, 134, 'ACCÈS:\r\nL\'échelle d\'accès n\'est pas fixée et/ou ne dépasse pas d\'au moins 1m le plan d\'accès.', 2, NULL, NULL),
(747, 233, 'UTILISATION ESCABEAU:\r\nIl a été indiqué que le ou les escabeaux sont utilisés comme poste de travail.', 2, NULL, NULL),
(748, 136, 'Les dispositifs d\'aide à la manutention sont insuffisants ou inadaptés.', 2, NULL, NULL),
(749, 232, 'Absence de gant de manutention manuelle.', 2, NULL, NULL),
(757, 239, 'TRACTOPELLE:\r\nLes agents qui utilisent la chargeuse-pelleteuse possèdent le CACES 4', 1, NULL, NULL),
(758, 239, 'TRACTOPELLE:\r\nLes agents qui utilisent la chargeuse/pelleteuse n’ont pas suivi de formation à son utilisation en sécurité de type CACES.', 2, NULL, NULL),
(759, 239, 'TRACTEUR>50CV:\r\nLes agents qui utilisent un tracteur agricole supérieur à 50 chevaux possèdent le CACES 8.', 1, NULL, NULL),
(760, 239, 'TRACTEUR>50CV:\r\nLes agents qui utilisent le tracteur agricole n’ont pas suivi de formation à son utilisation en sécurité de type CACES.', 2, NULL, NULL),
(761, 239, 'TRACTEUR<50CV/MINIPELLE:\r\nLes agents qui utilisent des engins de la catégorie 1 (tracteur agricole de puissance inférieure à 50 chevaux , minipelle, minichargeuse), possèdent le CACES correspondant.', 1, NULL, NULL),
(762, 239, 'PEMP:\r\nLes agents qui utilisent une PEMP (plate forme élévatrice mobile de personnel) possèdent le CACES 1B ou 3B.', 1, NULL, NULL),
(763, 239, 'TRACTEUR<50CV/MINIPELLE:\r\nLes agents qui utilisent des engins de la catégorie 1 (tracteur agricole de puissance inférieure à 50 chevaux , minipelle, minichargeuse), n\'ont pas suivi de formation à leur utilisation en sécurité de type CACES.', 2, NULL, NULL),
(764, 239, 'PEMP:\r\nLes agents qui utilisent une PEMP (plate forme élévatrice mobile de personnel) n\'ont pas suivi de formation à son utilisation en sécurité de type CACES.', 2, NULL, NULL),
(765, 157, 'TRACTEUR:\r\nLe tracteur est équipé d\'un gyrophare visible tout azimut.', 1, NULL, NULL),
(766, 157, 'TRACTEUR:\r\nLe tracteur n\'est pas équipé d\'un gyrophare.', 2, NULL, NULL),
(767, 157, 'TRACTOPELLE:\r\nLe tractopelle est équipé d\'un gyrophare visible tout azimut.', 1, NULL, NULL),
(768, 157, 'TRACTOPELLE:\r\nLe tractopelle n\'est pas équipé d\'un gyrophare.', 2, NULL, NULL),
(769, 157, 'SIGNALISATION COMPLÉMENTAIRE:\r\nLes engins de chantier considérés comme des véhicules à progression lente sont équipés de bandes rouges et blanches rétro réfléchissantes.', 1, NULL, NULL),
(770, 157, 'SIGNALISATION COMPLÉMENTAIRE:\r\nLes engins de chantier considérés comme des véhicules à progression lente ne sont pas tous équipés de bandes rouges et blanches rétro réfléchissantes.', 2, NULL, NULL),
(775, 240, 'Les organes de coupure d\'énergie (vannes "police") sont signalés et accessibles.', 1, NULL, NULL),
(776, 240, 'Les organes de coupure d\'énergie (vannes "police") ne sont pas accessibles.', 2, NULL, NULL),
(778, 176, 'COMPATIBILITÉ:\r\nLes règles se stockage en fonction des incompatibilités ne sont pas respectée.', 2, NULL, NULL),
(779, 176, 'COMPATIBILITÉ:\r\nLes produits stockés incompatibles sont stockés sur des rétentions séparées.', 1, NULL, NULL),
(780, 240, 'Les organes de coupure d\'énergie (vannes "police") ne sont pas signalés.', 2, NULL, NULL),
(781, 161, 'La ou les tondeuses autoportées ont un éclairage défaillant. (Gyrophare et/ou éclairage de position et/ou de changement de direction)', 2, NULL, NULL),
(782, 241, 'Le ou les agents ont été formés à l\'utilisation des artifices.', 1, NULL, NULL),
(783, 241, 'Le ou les agents n\'ont pas été formés à l\'utilisation des artifices.', 2, NULL, NULL),
(784, 242, 'Le ou les agents ont été habilités par l\'Autorité Territoriale.', 1, NULL, NULL),
(785, 242, 'Le ou les agents n\'ont pas été habilités par l\'Autorité Territoriale.', 2, NULL, NULL),
(786, 243, 'Le stockage est conforme à la réglementation.', 1, NULL, NULL),
(787, 243, 'Le stockage n\'est pas conforme à la réglementation.', 2, NULL, NULL),
(788, 169, 'Il a été indiqué que le ou les agents n’ont pas suivi de formation à l’utilisation et à la manipulation des produits dangereux.', 2, NULL, NULL),
(789, 170, 'PRÉSENCE:\r\nLes Fiches de données de sécurité des produits utilisés et stockés ne sont pas présentes sur les lieux de stockage et ne peuvent être consultées par les agents.', 2, NULL, NULL),
(790, 142, 'PORT OBLIGATOIRE:\r\nIl a été observé que les agents ne portent pas la partie haute du vêtement de signalisation, ce qui ne correspond pas à la classe 2 (le haut et le bas étant indissociable).', 2, NULL, NULL),
(791, 142, 'ETAT:\r\nIl a été observé que les vêtements sont, pour certains, dans un état de propreté et/ou d\'usure insuffisant pour les maintenir efficaces.', 2, NULL, NULL),
(792, 179, 'VÉRIFICATION:\r\nAbsence de vérification de la conformité de l\'installation électrique.', 2, NULL, NULL),
(793, 179, 'SUIVI:\r\nLa ou les installations ne sont pas conformes (vérification périodique avec des observations non levées)', 2, NULL, NULL),
(794, 182, 'PORT:\r\nLes agents disposent d\'équipements de protection individuelles contre les nuisances sonores et les portent.', 1, NULL, NULL),
(796, 4, 'FORMATION OBLIGATOIRE:\r\nIl a été indiqué que les agents désignés Assistant de Prévention n\'ont pas tous suivi la formation obligatoire.', 2, NULL, NULL),
(797, 4, 'FORMATION CONTINUE:\r\nIl a été indiqué que le ou les agents ont suivi la formation initiale mais ne sont pas à jour de sa formation continue d\'Assistant de Prévention.', 2, NULL, NULL),
(798, 9, 'FORMATION CONTINUE:\r\nIl a été indiqué que l\'agent a suivi la formation initiale mais n\'est pas à jour de sa formation continue.', 2, NULL, NULL),
(799, 36, 'Il a été indiqué que le registre Santé et Sécurité au Travail n\'a  pas été ouvert dans tous les services.', 2, NULL, NULL),
(800, 44, 'Il a été indiqué qu\'il n\'y a pas d\'agents formés et habilités en nombre suffisant pour intervenir sur les installations électriques ou à proximité.', 2, NULL, NULL),
(801, 48, 'Il a été indiqué qu\'il n\'y a pas de vérifications périodiques de l\'ensemble des installations d\'aération et de ventilation.', 2, NULL, NULL),
(802, 50, 'Il a été indiqué que certains appareils ou accessoires de levage ne sont pas contrôlés périodiquement.', 2, NULL, NULL),
(803, 230, 'Le ou les arbres à cardan ont fait l\'objet d\'une vérification périodique (annuelle)', 1, NULL, NULL),
(804, 230, 'Il a été indiqué que les arbres à cardan n\'ont pas tous fait l\'objet d\'une vérification périodique (annuelle)', 2, NULL, NULL),
(805, 58, 'PTAC<3,5T:\r\nLes contrôles techniques des véhicules (légers et/ou utilitaires) ne sont pas effectués.', 2, NULL, NULL),
(806, 58, 'PTAC>3,5T:\r\nLes contrôles techniques du ou des véhicules lourds ne sont pas effectués.', 2, NULL, NULL),
(807, 58, 'PTAC>3,5T:\r\nLes contrôles techniques du ou des véhicules lourds sont effectués semestriellement.', 1, NULL, NULL),
(808, 244, 'Le ou les véhicules en circulation sont équipés d’un triangle de présignalisation et d’un gilet rétroréfléchissant rangés dans l’habitacle et aux normes CE.', 1, NULL, NULL),
(809, 244, 'Le ou les véhicules en circulation ne sont pas équipés d’un triangle de présignalisation et d’un gilet rétroréfléchissant.', 2, NULL, NULL),
(810, 244, 'Les véhicules en circulation ne sont pas tous équipés d’un triangle de présignalisation et d’un gilet rétroréfléchissant.', 2, NULL, NULL),
(811, 53, 'SUIVI:\r\nIl a été indiqué que la vérification périodique des installations électriques était effectuée mais sans formalisation de la levée des non conformités relevées dans le rapport.', 2, NULL, NULL),
(812, 52, 'Il a été indiqué que tous les équipements de travail en hauteur ne sont pas vérifiés. (Voir la liste dans les préconisations)', 2, NULL, NULL),
(813, 52, 'SUIVI:\r\nIl a été indiqué que les équipements pour le travail en hauteur sont vérifiés mais ces informations ne sont pas formalisées sur un registre de sécurité.', 2, NULL, NULL),
(814, 62, 'Il a été indiqué que les formations initiales à la sécurité n\'était pas systématiquement effectuées.', 2, NULL, NULL),
(815, 63, 'Il a été indiqué qu\'aucun plan de formation n\'a été mis en place.', 2, NULL, NULL),
(816, 63, 'Un plan de formation a été mis en place.', 1, NULL, NULL),
(818, 64, 'Il a été indiqué que tous les agents n\'ont pas suivi de formation à l\'utilisation des extincteurs.', 2, NULL, NULL),
(819, 65, 'RECYCLAGE:\r\nIl a été indiqué que les agents formés SST (sauveteur secouriste du travail) ne sont pas tous à jour de leur recyclage obligatoire.', 2, NULL, NULL),
(820, 67, 'MINIPELLE/TRACTEUR<50CV:\r\nIl a été indiqué que pour la conduite en sécurité du ou des minipelles/tracteurs <50cv, le ou les agents n\'ont pas suivi de formation.', 2, NULL, NULL),
(821, 67, 'TRACTOPELLE:\r\nIl a été indiqué que pour la conduite en sécurité du ou des tractopelles, le ou les agents n\'ont pas suivi de formation.', 2, NULL, NULL),
(822, 67, 'PEMP:\r\nIl a été indiqué que pour la conduite en sécurité de la nacelle élévatrice, le ou les agents n\'ont pas suivi de formation.', 2, NULL, NULL),
(823, 67, 'TONDEUSE AUTOPORTEE:\r\nIl a été indiqué que pour la conduite en sécurité des tondeuses autoportées, le ou les agents n\'ont pas suivi de formation.', 2, NULL, NULL),
(824, 67, 'RECYCLAGE:\r\nIl a été indiqué que tous les agents ne sont pas à jour de leur recyclage CACES.', 2, NULL, NULL),
(825, 75, 'Il a été indiqué qu\'il n\'y a pas de procédure en place pour les interventions sous astreinte.', 2, NULL, NULL),
(826, 80, 'Le suivi et l\'entretien sont assurés par un prestataire. ', 1, NULL, NULL),
(827, 92, 'L\'autorité territoriale ne délivre pas de Permis de feu pour les travaux par "point chaud".', 2, NULL, NULL),
(828, 92, 'L\'autorité territoriale délivre un Permis de feu pour tous les travaux par "point chaud".', 1, NULL, NULL),
(830, 97, 'LAVABOS:\r\nAbsence de lavabo.', 2, NULL, NULL),
(831, 97, 'LAVABOS:\r\nLes lavabos ne sont pas munis de dispositif de nettoyage, d’essuyage et de séchage approprié.', 2, NULL, NULL),
(832, 97, 'ARMOIRES:\r\nLes vestiaires ne possèdent pas d’armoires individuelles.', 2, NULL, NULL),
(833, 206, 'Le ou les plans de travail situés en hauteur ne disposent pas de garde-corps réglementaires.', 2, NULL, NULL),
(834, 223, 'Des mesures de VLEP ont été effectuées et sont supérieures à la réglementation.', 2, NULL, NULL),
(835, 227, 'RADON:\r\nLa recherche de radon a été faite sur le ou les établissements scolaires', 1, NULL, NULL),
(836, 227, 'RADON:\r\nLa recherche de radon n\'a pas été faite sur le ou les établissements scolaires.', 2, NULL, NULL),
(837, 227, 'LÉGIONELLOSE:', 2, NULL, NULL),
(838, 227, 'LÉGIONELLOSE:', 1, NULL, NULL),
(840, 212, 'Il a été indiqué que les agents qui utilisent des produits phyto-sanitaires n\'ont pas tous suivi la formation Certi-Phyto. ', 2, NULL, NULL),
(841, 216, 'Il a été indiqué que les agents intervenants sur la fabrication ou la manutention des repas n\'ont pas tous suivi de formation  à l\'hygiène alimentaire (HACCP)', 2, NULL, NULL),
(842, 245, 'Les agents ont à leur disposition les EPI adaptés (gants anti-chaleur, gants anti-coupures, vêtements et chaussures de sécurité)', 1, NULL, NULL),
(843, 245, 'Absence de gant de protection contre la coupure.', 2, NULL, NULL),
(844, 96, 'AFFICHAGE:\r\nL’affichage réglementaire est présent.', 1, NULL, NULL),
(845, 118, 'MISE A JOUR:\r\nLes plans d\'évacuation sont incomplets.', 2, NULL, NULL),
(846, 78, 'Il a été indiqué que les agents ne sont pas tous dotés des équipements de travail et de protection individuelle nécessaire.', 2, NULL, NULL),
(847, 81, 'SENSIBILISATION:\r\nIl a été indiqué qu\'une information a été faite au personnel sur la nécessité du port des EPI.', 1, NULL, NULL),
(848, 81, 'SENSIBILISATION:\r\nAbsence d\'information sur la nécessité du port des EPI.', 2, NULL, NULL),
(849, 71, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(850, 1, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(851, 2, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(852, 3, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(853, 4, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
...
(913, 63, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(914, 64, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(916, 66, 'Il a été indiqué que les agents n\'ont pas tous suivi de formation sur les risques spécifiques sur leur poste de travail (voir liste dans la partie réglementaire).', 2, NULL, NULL),
(917, 67, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(918, 68, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(919, 69, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(920, 70, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
...
(940, 91, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(941, 92, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(944, 247, 'EVALUATION DES RISQUES:\r\nLe risque d\'effondrement et de chute d\'objet a fait l\'objet d\'une évaluation des risques dans le document unique.', 1, NULL, NULL),
(945, 247, 'EVALUATION DES RISQUES:\r\nLe risque d\'effondrement et de chute d\'objet  n\'a pas fait l\'objet d\'une évaluation des risques dans le document unique.', 2, NULL, NULL),
(946, 248, 'ECHELLE FIXE>3M:\r\nL\'échelle fixe supérieur à 3m est équipée d\'une crinoline.', 1, NULL, NULL),
(947, 248, 'ECHELLE FIXE>3M:\r\nL\'échelle fixe supérieur à 3m n\'est pas équipée d\'une crinoline.', 2, NULL, NULL),
(948, 248, 'ECHELLE D’ACCÈS:\r\nL\'échelle d\'accès dépasse d\'au moins un mètre le niveau d\'accès.', 1, NULL, NULL),
(949, 248, 'ECHELLE D’ACCÈS:\r\nL\'échelle d\'accès ne dépasse d\'au moins un mètre le niveau d\'accès.', 2, NULL, NULL),
(950, 66, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(951, 134, 'PORT DE CHARGES:\r\nDes charges sont transportées par l\'intermédiaire d\'une échelle d\'accès.', 2, NULL, NULL),
(952, 134, 'ACCÈS:\r\nL\'échelle d\'accès est fixée et dépasse d\'au moins 1m le plan d\'accès.', 1, NULL, NULL),
(953, 249, 'PNEUMATIQUES:\r\nLes pneumatiques du véhicule sont en bon état.\r\n', 1, NULL, NULL),
(954, 249, 'PNEUMATIQUES:\r\nLes pneumatiques du véhicule ont une dégradation excessive.', 2, NULL, NULL),
(955, 245, 'Absence de gant de protection contre la brûlure.', 2, NULL, NULL),
(956, 245, 'Absence de chaussures de sécurité fermées à embout renforcé.', 2, NULL, NULL),
(957, 250, 'Présence d\'une couverture anti-feu facilement accessible.', 1, NULL, NULL),
(958, 250, 'PRÉSENCE:\r\nAbsence de couverture anti-feu.', 2, NULL, NULL),
(959, 250, 'POSITIONNEMENT:\r\nLa couverture anti-feu n\'est pas positionnée de façon à la rendre facilement accessible.', 2, NULL, NULL),
(960, 251, 'MISE A DISPOSITION:\r\nDes équipements de protection individuelles contre les risques de chute d\'objet et d\'effondrement sont à disposition des agents.', 1, NULL, NULL),
(961, 251, 'MISE A DISPOSITION:\r\nIl n\'y a pas d\'équipement de protection individuelle contre les risques de chute d\'objet et d\'effondrement à disposition des agents.', 2, NULL, NULL),
(962, 247, 'EVALUATION DES RISQUES:\r\nL\'évaluation des risques d\'effondrement et de chute d\'objet n\'a pas pris en compte l\'ensemble des activités et situations des agents impliquant une exposition à ces risques.', 2, NULL, NULL),
(963, 251, 'ETAT:\r\nLes équipements  de protection individuelle à disposition des agents (casque, chaussures...) sont en bon état.', 1, NULL, NULL),
(964, 251, 'PORT:\r\nLes agents disposent d\'équipements de protection individuelles contre les risques de chute d\'objet et d\'effondrement et les portent.', 1, NULL, NULL),
(965, 251, 'ETAT:\r\nLes équipements  de protection individuelle à disposition des agents sont usagés ou détériorés.', 2, NULL, NULL),
(966, 251, 'PORT:\r\nLes agents disposent d\'équipements de protection individuelles contre les risques de chute d\'objet et d\'effondrement, mais il a été indiqué qu\'ils ne les portent pas systématiquement', 2, NULL, NULL),
(967, 249, 'ECLAIRAGE:\r\nL\'éclairage du véhicule est en état de fonctionnement.', 1, NULL, NULL),
(968, 249, 'ECLAIRAGE:\r\nL\'éclairage du véhicule est défaillant.', 2, NULL, NULL),
(969, 65, 'A CONTRÔLER:\r\nAbsence de document ou d\'information permettant d\'apprécier la conformité du critère.', 2, NULL, NULL),
(970, 80, 'Le nettoyage et le suivi sont assurés par l\'employeur.', 1, NULL, NULL),
(1006, 170, 'PRÉSENCE:\r\nLes Fiches de données de sécurité des produits utilisés et stockés sont présentes sur les lieux de stockage et peuvent être consultées par les agents.', 1, NULL, NULL),
(1012, 253, 'Il a été constaté la présence de boissons alcoolisées non réglementaires et accessibles aux agents ', 2, NULL, NULL),
(1013, 254, 'Il existe un règlement intérieur qui définit l\'ensemble des postes à risque et qui contient un protocole d\'action (procédure de gestion immédiate de ces situations)', 1, NULL, NULL),
(1015, 253, 'Il a été indiqué qu\'aucune boisson alcoolisée autre que le vin, la bière, le cidre et le poiré n\'est présente', 1, NULL, NULL),
(1016, 253, 'Il existe un règlement intérieur où sont intégrées les  obligations et interdictions en matière d\'alcool', 1, NULL, NULL),
(1017, 254, 'Il existe une procédure à mettre en oeuvre sans règlement intérieur pour les cas d\'agent en état d\'ébriété.', 1, NULL, NULL),
(1018, 254, 'Il n\'existe pas de règlement intérieur  ni de note de service qui définit l\'ensemble des postes à risque et qui contient un protocole d\'action (procédure de gestion immédiate de ces situations)', 2, NULL, NULL),
(1023, 94, 'ENCOMBREMENT: \r\nLes locaux de travail sont exempts de tout encombrement.', 1, NULL, NULL);

--
-- Déclencheurs `observation`
--
DELIMITER $$
CREATE TRIGGER `incre_observation` BEFORE INSERT ON `observation` FOR EACH ROW BEGIN
	DECLARE increobservation INTEGER;
	SET increobservation = (SELECT COALESCE(MAX(NUM_OBSERVATION), 0) + 1 FROM observation);
	SET NEW.NUM_OBSERVATION = increobservation;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_participant` BEFORE INSERT ON `participant` FOR EACH ROW BEGIN
	DECLARE increparticipant INTEGER;
	SET increparticipant = (SELECT COALESCE(MAX(NUM_PARTICIPANT), 0) + 1 FROM participant);
	SET NEW.NUM_PARTICIPANT = increparticipant;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_pole` BEFORE INSERT ON `pole` FOR EACH ROW BEGIN
	DECLARE increpole INTEGER;
	SET increpole = (SELECT COALESCE(MAX(CODE_POLE), 0) + 1 FROM pole);
	SET NEW.CODE_POLE = increpole;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_preconisation` BEFORE INSERT ON `preconisation` FOR EACH ROW BEGIN
	DECLARE increpreconisation INTEGER;
	SET increpreconisation = (SELECT COALESCE(MAX(NUM_PRECONISATION), 0) + 1 FROM preconisation);
	SET NEW.NUM_PRECONISATION = increpreconisation;
END
$$
DELIMITER ;
...
--
-- Déclencheurs `resume_article`
--
DELIMITER $$
CREATE TRIGGER `incre_resumer_article` BEFORE INSERT ON `resume_article` FOR EACH ROW BEGIN
	DECLARE increresumerarticle INTEGER;
	SET increresumerarticle = (SELECT COALESCE(MAX(NUM_RESUME_ARTICLE), 0) + 1 FROM resume_article);
	SET NEW.NUM_RESUME_ARTICLE = increresumerarticle;
END
$$
DELIMITER ;
...
--
-- Déclencheurs `sous_theme`
--
DELIMITER $$
CREATE TRIGGER `incre_sous_theme` BEFORE INSERT ON `sous_theme` FOR EACH ROW BEGIN
	DECLARE incresoustheme INTEGER;
	SET incresoustheme = (SELECT COALESCE(MAX(NUM_SOUS_THEME), 0) + 1 FROM sous_theme);
	SET NEW.NUM_SOUS_THEME = incresoustheme;
END
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `incre_theme` BEFORE INSERT ON `theme` FOR EACH ROW BEGIN
	DECLARE incretheme INTEGER;
	SET incretheme = (SELECT COALESCE(MAX(NUM_THEME), 0) + 1 FROM theme);
	SET NEW.NUM_THEME = incretheme;
END
$$
DELIMITER ;
