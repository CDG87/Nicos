...
--
-- Client :  127.0.0.1
-- Généré le :  Mar 16 Janvier 2018 à 10:19
...
--
DELIMITER $$
CREATE TRIGGER `incre_structure` BEFORE INSERT ON `structure` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `incre_type_structure` BEFORE INSERT ON `type_structure` FOR EACH ROW BEGIN
...
ALTER TABLE `audit`
  ...
  ADD KEY `I_FK_AUDIT_CONTROLEUR` (`NUM_CONTROLEUR`),
  ADD KEY `I_FK_AUDIT_STRUCURE` (`NUM_STRUCTURE`);
...
ALTER TABLE `structure`
  ...
  ADD KEY `I_FK_STRUCURE_TYPE_STRUCTURE` (`NUM_TYPE_STRUCTURE`);
